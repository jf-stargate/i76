...
Important corrections in v2/v3:
  v2: mpack_database_get_item_alloc_or_alias (i76shell.dll 10008320) does not return raw
  ...
  v3: confirmed trailing palette should be treated as RGB8 source bytes and quantized
      through the shell path (>>2 then display expansion). The previous rgb6 mode is
      retained only for diagnostics and is no longer emitted by default.
...
def main() -> int:
    ...
    ap.add_argument("--palette-mode", choices=["rgb8_trailing", "rgb6_trailing", "both"], default="rgb8_trailing", help="Default rgb8_trailing matches i76shell.dll 1003ea06: source byte >> 2, later displayed as RGB6-expanded color. rgb6_trailing is diagnostic only.")
    ...
    (out / "summary.json").write_text(json.dumps({
        ...
        "confirmed_palette_interpretation": "rgb8_trailing; rgb6_trailing is diagnostic/blown-out for confirmed shell surfaces",
