#!/usr/bin/env python3
"""
i76_vehicle_geo_obj_export_v8.py

Vehicle GEO OBJ/MTL exporter using resolved texture slots and repaired VDF part transforms.
...
assembler:
  ...
  * It applies VDF VGEO hierarchy transforms and conservatively repairs missing damage-state parent references.
...
Usage:
  python3 i76_vehicle_geo_obj_export_v8.py \
      --zfs I76.ZFS \
      --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
      --out i76_vehicle_geo_obj_v8 \
      --vehicle vcpolic1 \
      --export-textures \
      --overwrite

Export all vehicles with VDF/VTF:
  python3 i76_vehicle_geo_obj_export_v8.py --zfs I76.ZFS --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
      --out i76_vehicle_geo_obj_v8 --all --overwrite
"""
...
def parse_vdf_geometry_transforms(data: bytes, damage_state: int = 0) -> Tuple[List[PartTransform], List[str]]:
    ...
    Retail data sometimes leaves child parent names pointing at a damage-state
    sibling from an earlier state, for example SV11FWLL -> parent SV11BDYF while
    the active damage-state body part is SV13BDYF.  Open76 confirms that VGEO is
    stored as four independent third-person part sets, so this function repairs
    only exact same-prefix/same-suffix parent substitutions inside the selected
    active part set.  The repair is deliberately conservative and is recorded as
    a "repair_parent ..." diagnostic rather than silently hidden.
    """
    ...
    def repaired_damage_parent_key(parent_key: str) -> str:
        """Return an in-set damage-state equivalent for a missing parent.

        This intentionally recognizes only the observed 8-char vehicle part
        convention: two-character family prefix, two damage/variant digits,
        and a stable suffix such as BDYF/BDYB.  Example:
          SV11BDYF -> SV13BDYF for damage_state 2.
        """
        parent_key = parent_key.upper()
        if damage_state <= 0 or len(parent_key) < 5:
            return ""
        if not parent_key[2:4].isdigit():
            return ""

        expected = parent_key[:2] + f"{11 + damage_state:02d}" + parent_key[4:]
        if expected in locals_by_name:
            return expected

        candidates = [
            k for k in locals_by_name.keys()
            if len(k) == len(parent_key)
            and k[:2] == parent_key[:2]
            and k[4:] == parent_key[4:]
            and k[2:4].isdigit()
            and k != parent_key
        ]
        if len(candidates) == 1:
    ...
    def resolve_key(key: str, stack: Optional[List[str]] = None) -> PartTransform:
        ...
        if parent_key and parent_key not in locals_by_name:
            repaired_key = repaired_damage_parent_key(parent_key)
            if repaired_key:
                warnings.append(
                    f"repair_parent child={local.name!r} old_parent={local.parent_name!r} new_parent={locals_by_name[repaired_key].name!r}"
                )
                effective_local = PartTransform(
                    name=local.name,
                    parent_name=locals_by_name[repaired_key].name,
                    right=local.right,
                    up=local.up,
                    forward=local.forward,
                    position=local.position,
                    world_right=local.world_right,
                    world_up=local.world_up,
                    world_forward=local.world_forward,
                    world_position=local.world_position,
                    source=local.source + "+parent_repair",
                )
                parent_key = repaired_key
        ...
        if not parent_key or parent_key not in locals_by_name:
            ...
            resolved[key] = effective_local
            return effective_local

        parent_world = resolve_key(parent_key, stack + [key])
        resolved[key] = compose_basis(parent_world, effective_local)
        return resolved[key]
...
def main() -> int:
    ...
    for vcf in vcf_assets:
        ...
        parent_repair_notes = [w for w in transform_warnings if w.startswith("repair_parent ")]
        transform_errors = [w for w in transform_warnings if not w.startswith("repair_parent ")]
        ...
        if part_rows:
            ...
            vehicle_rows.append({
                ...
                "parent_repair_count": len(parent_repair_notes),
                "parent_repairs": "; ".join(parent_repair_notes[:40]),
                "parse_errors": "; ".join((([vcf_ref_notes] if vcf_ref_notes else []) + parse_errors)[:20]),
            })
        else:
            vehicle_rows.append({"vehicle": vcf.name, "status": "no_geo_parts", "vdf": vdf.name, "vtf": vtf.name, "parts": 0, "faces": 0, "materials": 0, "parent_repair_count": len(parent_repair_notes), "parent_repairs": "; ".join(parent_repair_notes[:40]), "parse_errors": "; ".join(parse_errors[:20])})

    write_tsv(args.out / "vehicle_export_summary.tsv", vehicle_rows, ["vehicle", "status", "vdf", "vtf", "parts", "faces", "materials", "obj", "mtl", "parent_repair_count", "parent_repairs", "parse_errors"])
    if face_rows:
    ...
    summary = {
        ...
        "parent_repairs": sum(int(r.get("parent_repair_count", 0) or 0) for r in vehicle_rows),
    ...
    (args.out / "README.txt").write_text("""I76 vehicle GEO OBJ exporter v8
...
Important:
  ...
  * Missing damage-state parent references are conservatively repaired when a unique same-prefix/same-suffix active-state parent exists.
