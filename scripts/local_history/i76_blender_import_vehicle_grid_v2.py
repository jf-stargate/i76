"""
i76_blender_import_vehicle_grid_v2.py
...
# is already suitable.
ROTATION_EULER_DEGREES = (0.0, 0.0, -90.0)
...
RUN_VEHICLE_MATERIAL_SCRIPT = False
...
def clear_scene() -> None:
    ...
    # Remove collections created by previous runs.
    ...
    # Re-running a large import without purging orphan data-blocks causes Blender
    # to retain old meshes/materials/images. That makes the second run appear to
    # hang, especially when material setup loops over all historical materials.
    for datablocks in (
        bpy.data.meshes,
        bpy.data.curves,
        bpy.data.materials,
    ...
    # Blender 3.x/4.x orphan purge catches additional unused node groups, actions, etc.
    for _ in range(3):
        try:
...
def collection_has_child(parent: bpy.types.Collection, child: bpy.types.Collection) -> bool:
    return any(c.name == child.name for c in parent.children)
...
def ensure_collection(name: str, parent: Optional[bpy.types.Collection] = None) -> bpy.types.Collection:
    ...
    if parent is None:
        if not collection_has_child(bpy.context.scene.collection, coll):
            try:
                bpy.context.scene.collection.children.link(coll)
            except RuntimeError:
                pass
    else:
        if not collection_has_child(parent, coll):
            try:
