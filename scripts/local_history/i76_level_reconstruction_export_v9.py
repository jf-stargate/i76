#!/usr/bin/env python3
"""
Interstate '76 level reconstruction exporter, v9.
...
def parse_sdf_sgeo_parts(sdf_path: Path, file_index: Dict[str, List[Path]], include_extra_records: bool = True) -> List[dict]:
...
def sdf_child_candidates(sdf_path: Path, file_index: Dict[str, List[Path]], max_rows: int = 2048) -> List[dict]:
    ...
    rows = parse_sdf_sgeo_parts(sdf_path, file_index, include_extra_records=True)
    if rows:
...
def export(args: argparse.Namespace) -> dict:
    ...
    out_dir = timestamped_out_dir(args.out_dir, "i76_level_reconstruction_export_v9")
    ...
    write_tsv(
        ...
        all_sdf_rows,
        ["mission", "object_label", "object_index", "object_class_hex", "sdf_path", "sdf_object_name", "sgeo_chunk_offset_hex", "declared_part_count", "offset_hex", "parser", "part_kind", "part_index", "token", "parent_name", "geo_name", "geo_found", "geo_path", "score", "reasons", "right_x", "right_y", "right_z", "up_x", "up_y", "up_z", "forward_x", "forward_y", "forward_z", "pos_x", "pos_y", "pos_z"],
    )
    write_tsv(
    ...
    notes = [
        "i76_level_reconstruction_export_v9 notes",
        "",
