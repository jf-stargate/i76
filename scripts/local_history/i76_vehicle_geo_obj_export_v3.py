#!/usr/bin/env python3
"""
i76_vehicle_geo_obj_export_v3.py
...
Usage:
  python3 i76_vehicle_geo_obj_export_v3.py \
      --zfs I76.ZFS \
      --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
      --out i76_vehicle_geo_obj_v3 \
      --vehicle vcpolic1 \
      --export-textures \
      --overwrite

Export all vehicles with VDF/VTF:
  python3 i76_vehicle_geo_obj_export_v3.py --zfs I76.ZFS --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
      --out i76_vehicle_geo_obj_v3 --all --overwrite
"""
...
def main() -> int:
    ...
    ap.add_argument("--tar", action="store_true", help="Also create sibling .tar.gz")
    args = ap.parse_args()
    ...
    for vcf in vcf_assets:
        ...
        vdf, vtf, _wdfs, _gdfs = resolver.parse_vcf_refs(vcf, vcf_data, idx)
        if not vdf or not vtf:
            vehicle_rows.append({"vehicle": vcf.name, "status": "missing_vdf_or_vtf", "vdf": "", "vtf": "", "parts": 0, "faces": 0, "materials": 0})
            continue
    ...
    summary = {
        ...
        "notes": "VDF SOBJ/VGEO transforms are applied for the selected third-person damage state.",
    }
...
Important:
  ...
  * VTF/TMT texture list interpretation follows Open76 VtfParser:
    ...
    8-byte strings.
  * Parts are transformed through VDF SOBJ/VGEO hierarchy in v3.
