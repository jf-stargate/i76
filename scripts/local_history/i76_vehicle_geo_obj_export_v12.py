#!/usr/bin/env python3
"""
i76_vehicle_geo_obj_export_v12.py
...
Usage:
  python3 i76_vehicle_geo_obj_export_v12.py \
      --zfs I76.ZFS \
      --zix I76.ZIX \
...
Export all vehicles with VDF/VTF:
  python3 i76_vehicle_geo_obj_export_v12.py --zfs I76.ZFS --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
...
def main() -> int:
    ...
    ap.add_argument("--shadow-object-mode", choices=["include", "skip"], default="include", help="How to handle *_SOR vehicle shadow objects. include preserves current behavior; skip omits them from OBJ/MTL export.")
    ...
    for vcf in vcf_assets:
        ...
        skipped_shadow_parts = 0
        skipped_shadow_faces = 0
        skipped_shadow_names = []
        for part_transform in transforms:
            ...
            try:
                mesh = parse_geo(data)
                if args.shadow_object_mode == "skip" and (is_shadow_object_name(part_transform.name) or is_shadow_object_name(geo_asset.name) or is_shadow_object_name(mesh.name)):
                    skipped_shadow_parts += 1
                    skipped_shadow_faces += len(mesh.faces)
                    if len(skipped_shadow_names) < 40:
                        skipped_shadow_names.append(part_transform.name)
                    continue
        ...
        if part_rows:
            ...
            vehicle_rows.append({
                ...
                "skipped_shadow_parts": skipped_shadow_parts,
                "skipped_shadow_faces": skipped_shadow_faces,
                "skipped_shadow_names": ";".join(skipped_shadow_names),
            ...
            vehicle_rows.append({"vehicle": vcf.name, "status": "no_geo_parts", "vdf": vdf.name, "vtf": vtf.name, "parts": 0, "faces": 0, "materials": 0, "skipped_shadow_parts": skipped_shadow_parts, "skipped_shadow_faces": skipped_shadow_faces, "skipped_shadow_names": ";".join(skipped_shadow_names), "alternate_state_note_count": len(alternate_state_notes), "alternate_state_notes": "; ".join(alternate_state_notes[:60]), "parse_errors": "; ".join(parse_errors[:20])})

    write_tsv(args.out / "vehicle_export_summary.tsv", vehicle_rows, ["vehicle", "status", "vdf", "vtf", "parts", "faces", "materials", "skipped_shadow_parts", "skipped_shadow_faces", "skipped_shadow_names", "obj", "mtl", "alternate_state_note_count", "alternate_state_notes", "parse_errors"])
    if face_rows:
    ...
    if material_rows:
        write_tsv(args.out / "materials.tsv", material_rows, ["vehicle", "material", "name", "raw_slot", "normalized_slot", "resolved_kind", "resolved_name", "resolved_asset_id", "texture_asset_name", "texture_asset_id", "confidence", "evidence", "color", "diffuse_color", "diffuse_source", "palette_index", "map_kd"])

    summary = {
        "zfs_header": header,
        "damage_state": args.damage_state,
        "flat_color_mode": args.flat_color_mode,
        "shadow_object_mode": args.shadow_object_mode,
        "skipped_shadow_parts": sum(int(r.get("skipped_shadow_parts", 0) or 0) for r in vehicle_rows),
        "skipped_shadow_faces": sum(int(r.get("skipped_shadow_faces", 0) or 0) for r in vehicle_rows),
