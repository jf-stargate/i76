#!/usr/bin/env python3
"""
i76_vehicle_geo_obj_export_v15.py
...
Usage:
  python3 i76_vehicle_geo_obj_export_v15.py \
      --zfs I76.ZFS \
      --zix I76.ZIX \
...
Export all vehicles with VDF/VTF:
  python3 i76_vehicle_geo_obj_export_v15.py --zfs I76.ZFS --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
...
def write_obj_mtl(vehicle_dir: Path, vehicle_name: str, part_rows: List[Tuple], export_textures: bool, resolver, idx, palette_rgb: Optional[bytes] = None, flat_color_mode: str = "auto", vqm_layout: str = "yx"):
...
def main() -> int:
    ...
    for vcf in vcf_assets:
        ...
        if sobj_aux_rows:
            ...
            sobj_obj_path, sobj_mtl_path, sobj_materials, sobj_report = write_obj_mtl(
                aux_dir, aux_name, sobj_aux_rows, args.export_textures, resolver, idx, palette_rgb, flat_color_mode_for_aux, args.vqm_layout
            )
        ...
        if part_rows:
            ...
            obj_path, mtl_path, materials, report = write_obj_mtl(vdir, vcf.name, part_rows, args.export_textures, resolver, idx, palette_rgb, flat_color_mode, args.vqm_layout)
    ...
    summary = {
        ...
        "notes": "VDF VGEO state-0 hierarchy is applied; alternate damage GEOs are bound by VGEO slot index; VTF refs use exact plus leading-prefix fallback; VQM decoding defaults to yx intra-tile layout.",
    }
    (args.out / "summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True), encoding="utf-8")
    (args.out / "README.txt").write_text("""I76 vehicle GEO OBJ exporter v15
