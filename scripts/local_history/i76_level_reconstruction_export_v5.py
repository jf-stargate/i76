...
# -*- coding: utf-8 -*-
"""
i76_level_reconstruction_export_v5.py

Interstate '76 level reconstruction exporter, v5.
...
v5 diagnostic pass now, and as a drop-in scaffold for importing v4's exact TER,
...
Examples
--------
    python3 i76_level_reconstruction_export_v5.py \
...
def parse_odef_records(path: Path) -> List[OdefRecord]:
    ...
    # records, also test common alignments and choose the one with the most sane
    # labels/transforms.
    candidates = [0]
    ...
    def score_offset(start: int) -> int:
        ...
        for off in range(start, len(data) - ODEF_RECORD_SIZE + 1, ODEF_RECORD_SIZE):
            ...
            label = decode_i76_label(rec[0:8])
            cls = rec[0x5C] if len(rec) > 0x5C else 0
            floats = struct.unpack_from("<12f", rec, 0x08)
            if label and all((c.isalnum() or c in "_-$#@!~") for c in label):
                score += 4
            if cls in VCF_CLASSES or cls in SDF_CLASSES or cls in NULLISH_OR_SPECIAL_CLASSES or cls == 0:
                score += 1
            if is_probably_float_tuple(floats):
                score += 2
            count += 1
            if count >= 64:
                break
    ...
    for off in range(best_start, len(data) - ODEF_RECORD_SIZE + 1, ODEF_RECORD_SIZE):
        ...
        # Keep invalid rows too; they are useful for diagnosing headers, class 0,
        # and blank/special rows.
        records.append(OdefRecord(path, idx, off, raw_label, label, basis, pos, cls, valid))
        idx += 1
    return records
...
def find_level_record_files(level_root: Optional[Path], explicit_files: Sequence[Path]) -> List[Path]:
    ...
    if level_root and level_root.exists():
        for p in level_root.rglob("*"):
            ...
            ext = p.suffix.lower()
            nm = p.name.lower()
            # I76 level object definitions may be dumped as .OBJ even though OBJ
            # also means Wavefront.  Avoid obvious Wavefront text files.
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Interstate '76 level reconstruction exporter v5")
    ...
    ap.add_argument("--odef-file", action="append", type=Path, default=[],
                    help="Explicit ODEF/OBJ binary record file. May be repeated.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    record_files = find_level_record_files(args.level_root, list(args.odef_file) + list(args.ldef_file))
...
Key outputs:
    ...
    object_records.tsv              Raw aligned ODEF/LDEF-style records.
