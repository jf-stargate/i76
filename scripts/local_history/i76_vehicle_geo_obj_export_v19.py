#!/usr/bin/env python3
"""
i76_vehicle_geo_obj_export_v19.py
...
Usage:
  python3 i76_vehicle_geo_obj_export_v19.py \
      --zfs I76.ZFS \
      --zix I76.ZIX \
...
Export all vehicles with VDF/VTF:
  python3 i76_vehicle_geo_obj_export_v19.py --zfs I76.ZFS --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
...
def resolve_material_asset(resolver, idx, geo_asset, raw_slot: str, tmt_assets, map_assets, map_ref_stems, direct_map_prefer: str = "current"):
        ...
        for item in part_rows:
            ...
            for face in geo_mesh.faces:
                ...
                resolved_for_uv, _kind_for_uv, _confidence_for_uv, _evidence_for_uv, norm_for_uv = resolve_material_asset(
                    resolver, idx, geo_asset, raw_slot_for_uv, tmt_assets, map_assets, map_ref_stems, direct_map_prefer
                )
            ...
            for face_idx, face in enumerate(geo_mesh.faces):
                ...
                resolved, kind, confidence, evidence, norm = resolve_material_asset(
                    resolver, idx, geo_asset, raw_slot, tmt_assets, map_assets, map_ref_stems, direct_map_prefer
                )
                texture_asset = None
                material_key = ""
                if resolved:
...
def main() -> int:
    ...
    for vcf in vcf_assets:
        ...
        if part_rows:
            ...
            obj_path, mtl_path, materials, report = write_obj_mtl(vdir, vcf.name, part_rows, args.export_textures, resolver, idx, palette_rgb, flat_color_mode, args.vqm_layout, args.direct_map_prefer, args.textured_kd_mode, args.uv_flip_u_slot, args.uv_flip_u_texture, args.uv_flip_u_material, args.reverse_face_order_slot, args.reverse_face_order_texture, args.reverse_face_order_material)
    ...
    (args.out / "README.txt").write_text("""I76 vehicle GEO OBJ exporter v19
