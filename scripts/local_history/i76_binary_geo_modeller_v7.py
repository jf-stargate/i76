#!/usr/bin/env python3
"""
i76_binary_geo_modeller_v7.py

Sixth-pass Interstate '76 binary-faithful vehicle GEO modeller with validated mirror-X OBJ view and polygon-color textured materials by default.
...
Validation note:
  ...
  material binding, or global UV flip.  v7 defaults to the validated mirror-X negative-determinant OBJ view transform while leaving the runtime model in original game coordinates. It also restores GEO polygon face color as the default textured material Kd and splits OBJ/MTL materials by face color by default, because MTL Kd is material-level rather than face-level.
"""
...
def export_texture_for_asset_oriented(helper, resolver, idx, texture_asset, tex_dir: Path, written: Dict[str, str],
                                      palette_rgb: Optional[bytes], vqm_layout: str, texture_image_transform: str) -> str:
    """Export decoded M16/VQM texture with optional final image-space flip.

    The old helper exporter is still used for the actual image writer and the
    validated M16/VQM parsers, but we decode here so that M16 and VQM can both
    be subjected to the same final image orientation probe.
    """
    cache_key = texture_asset.asset_id + "|image_transform=" + texture_image_transform + "|vqm_layout=" + vqm_layout
    ...
    if texture_asset.ext == "m16":
        ...
        if parsed:
            ...
            image_name = helper.write_indexed_image(out_base, w, h, pix, palette_rgb)
    ...
    written[cache_key] = "textures/" + image_name
    return written[cache_key]
...
def add_material(helper, resolver, idx, tex_dir: Path, materials: Dict[str, Dict[str, object]], mat_info,
                 ...
                 textured_kd_mode: str, flat_color_mode: str, texture_image_transform: str):
    ...
    if export_textures and texture_asset:
        map_kd = export_texture_for_asset_oriented(helper, resolver, idx, texture_asset, tex_dir, texture_written, palette_rgb, vqm_layout, texture_image_transform)
...
def write_runtime_obj_mtl(helper, resolver, idx, vehicle_dir: Path, vehicle_name: str, part_models: List[Dict[str, object]],
                          ...
                          render_corner_order: str, texture_image_transform: str, obj_geometry_transform: str):
        ...
        for pm in part_models:
            ...
            for sec in sections:
                ...
                add_material(helper, resolver, idx, tex_dir, materials, mat_info, face.color, palette_rgb,
                             export_textures, vqm_layout, texture_written, textured_kd_mode, flat_color_mode, texture_image_transform)
...
def main() -> int:
    ...
    ap.add_argument("--textured-kd-mode", choices=["white", "geo-direct", "palette-nearest"], default="geo-direct", help="Diffuse Kd for textured OBJ/MTL materials. Default geo-direct restores GEO polygon face color; white preserves the prior neutral texture-only preview; palette-nearest quantizes polygon color through the active palette.")
    ...
    for vcf in vcf_assets:
        ...
        if part_models:
            obj_path, mtl_path, materials, face_rows, corner_rows = write_runtime_obj_mtl(
                ...
                args.texture_image_transform, args.obj_geometry_transform,
            )
...
Orientation probes:
  ...
  --obj-geometry-transform identity|mirror-x|mirror-y|mirror-z|blender-xzy|blender-xzy-rh|i76-yup-to-blender-zup changes only the final OBJ coordinate view. v5 defaults to mirror-x, preserving the runtime model while applying the validated negative-determinant OBJ view without swapping axes. Use mirror-z if it matches your preferred vehicle facing direction better.
""", encoding="utf-8")
