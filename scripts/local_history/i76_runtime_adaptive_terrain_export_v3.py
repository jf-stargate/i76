...
def write_obj(path: Path, mission: Path, ter: Path, zmap: ZMap, b: AdaptiveBuilder, args: argparse.Namespace) -> None:
    ...
    with path.open('w', encoding='utf-8') as f:
        f.write("# Interstate '76 runtime-shaped adaptive terrain OBJ v3\n")
...
def out_dir_for(args: argparse.Namespace) -> Path:
    if args.out_dir is not None:
        return args.out_dir
    return args.default_root / time.strftime('i76_runtime_adaptive_terrain_v2_%Y%m%d_%H%M%S')
...
def export(args: argparse.Namespace) -> dict:
    ...
    manifest = {
        'format': 'i76_runtime_adaptive_terrain_export_manifest', 'version': 2,
    ...
    for mission in sorted(missions):
        try:
            ...
            builder.build()
            obj_path = obj_dir / f'{mission.stem.lower()}.runtime_adaptive_v2.obj'
    ...
    manifest_path = out_dir / 'i76_runtime_adaptive_terrain_export_manifest_v2.json'
...
def parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Export Interstate '76 runtime-shaped adaptive terrain OBJ diagnostics v2 from miss8/miss16 missions.")
