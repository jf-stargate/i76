#!/usr/bin/env python3
"""
i76_vehicle_geo_obj_export_v20.py
...
assembler:
  * It applies VDF VGEO part transforms where available.
...
Usage:
  python3 i76_vehicle_geo_obj_export_v20.py \
      --zfs I76.ZFS \
      --zix I76.ZIX \
...
Export all vehicles with VDF/VTF:
  python3 i76_vehicle_geo_obj_export_v20.py --zfs I76.ZFS --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
...
def vqm_tile_index(layout: str, sx: int, sy: int) -> int:
    """Map 4x4 CBK tile coordinates to a byte index.

    Based on the decode-variant probe, the old Open76-inspired xy family
    produces the visibly incorrect/aliased vehicle textures. Default exporter
    behavior is therefore switched to the yx family, with override options for
    targeted validation.
    """
    x = sx
    y = sy
    ...
    if "flipy" in layout:
        y = 3 - y
    base = layout
    if base.endswith('_flipx_flipy'):
        base = base[:-12]
    elif base.endswith('_flipx'):
        base = base[:-6]
    elif base.endswith('_flipy'):
        base = base[:-6]
    if base == 'xy':
        return x * 4 + y
    if base == 'yx':
        return y * 4 + x
    raise ValueError(f'unknown VQM layout: {layout}')


def decode_vqm_pixels(data: bytes, cbk_data: bytes, layout: str = 'yx') -> Optional[Tuple[int, int, bytes]]:
        ...
        for bx in range(tiles_x):
            ...
            for sy in range(4):
                ...
                for sx in range(4):
                    ...
                    if px >= w:
                        continue
                    pixels[py * w + px] = tile[vqm_tile_index(layout, sx, sy)]
...
def nearest_palette_index_rgb(palette_rgb: Optional[bytes], rgb: Tuple[int, int, int]) -> Optional[int]:
    """Return nearest runtime palette index by squared RGB distance.

    Binary evidence:
      construct_geometry_from_geo_buffer stores raw GEO face RGB bytes at runtime section
      +0x09/+0x0a/+0x0b and also stores cache_nearest_palette_cube_index(r,g,b) at +0x08.
      build_lit_geo_render_records_from_sections copies +0x08 to render record +0x1c.

      004796a0 iterates sample bytes from 006432e0 through, but not including,
      006435dd: exactly 255 RGB triplets. Therefore index 255 is excluded from
      the runtime nearest-color search, likely reserving it for a special use.
    """
    if palette_rgb is None or len(palette_rgb) < 768:
        return None
    r, g, b = rgb
    best_i = 0
    best_d = 1 << 62
    for i in range(255):
        off = i * 3
        dr = r - palette_rgb[off]
        dg = g - palette_rgb[off + 1]
        db = b - palette_rgb[off + 2]
        d = dr * dr + dg * dg + db * db
        if d < best_d:
            best_d = d
            best_i = i
...
def palette_rgb_at(palette_rgb: Optional[bytes], idx: Optional[int]) -> Optional[Tuple[int, int, int]]:
    if palette_rgb is None or idx is None or len(palette_rgb) < 768:
        return None
    off = int(idx) * 3
    return (palette_rgb[off], palette_rgb[off + 1], palette_rgb[off + 2])


def split_csv_patterns(values: List[str]) -> List[str]:
    out = []
    for v in values or []:
...
def pattern_matches_any(value: str, patterns: List[str]) -> bool:
    value_u = (value or "").upper()
    for pat in patterns:
        pat_u = (pat or "").upper()
        if pat_u and pat_u in value_u:
            return True
    return False


def should_flip_u_for_face(raw_slot: str, norm: str, material_key: str, texture_name: str,
                           flip_slots: List[str], flip_textures: List[str], flip_materials: List[str]) -> bool:
    return (
        pattern_matches_any(raw_slot, flip_slots) or
        pattern_matches_any(norm, flip_slots) or
        pattern_matches_any(texture_name, flip_textures) or
        pattern_matches_any(material_key, flip_materials)
    )


def should_reverse_face_order_for_face(raw_slot: str, norm: str, material_key: str, texture_name: str,
                                       reverse_slots: List[str], reverse_textures: List[str], reverse_materials: List[str]) -> bool:
    return (
        pattern_matches_any(raw_slot, reverse_slots) or
        pattern_matches_any(norm, reverse_slots) or
        pattern_matches_any(texture_name, reverse_textures) or
        pattern_matches_any(material_key, reverse_materials)
    )


def parse_slot_swap_specs(values: List[str]) -> Dict[str, str]:
    """Parse swap specs like BKLT:BKRT or BKLT=BKRT into an uppercase dict."""
    out: Dict[str, str] = {}
    for v in values or []:
        for part in str(v).split(","):
            p = part.strip()
            ...
            if ":" in p:
                ...
                raise ValueError(f"bad --swap-normalized-slot spec {p!r}; expected FROM:TO")
            a = a.strip().upper()
            b = b.strip().upper()
            if not a or not b:
                raise ValueError(f"bad --swap-normalized-slot spec {p!r}; expected FROM:TO")
...
def apply_normalized_slot_swap(norm: str, swap_map: Dict[str, str]) -> Tuple[str, str]:
    n = (norm or "").upper()
    if n in swap_map:
        return swap_map[n], f"normalized_slot_swapped:{n}->{swap_map[n]}"
    return norm, ""
...
def resolve_direct_texture_with_preference(resolver, raw_slot: str, norm: str, idx, geo_asset, direct_map_prefer: str):
    """Resolve literal GEO texture slots with an optional M16/VQM sibling preference.

    This is intentionally conservative. The preference only applies to literal
    direct slots such as PP11BKU1.MAP, not semantic multi-word VTF/TMT slots
    such as "V1 FT TP.MAP".
    """
    if direct_map_prefer == "current":
        return resolver.resolve_slot_as_direct_texture(raw_slot, norm, idx, geo_asset), "current"

    raw = (raw_slot or "").strip()
    # Avoid hijacking semantic slots that contain spaces and should normally
    # resolve through VTF/TMT/MAP logic.
    if " " in raw:
        return resolver.resolve_slot_as_direct_texture(raw_slot, norm, idx, geo_asset), "current_semantic_slot"

    stem = raw.rsplit(".", 1)[0] if "." in raw else norm
    stems = []
    for s in [stem, norm]:
        s = (s or "").strip()
        if s and s.upper().startswith("COLOR") is False and s not in stems:
            stems.append(s)

    if direct_map_prefer == "vqm":
        order = ["vqm", "m16"]
    elif direct_map_prefer == "m16":
        order = ["m16", "vqm"]
    else:
        order = ["m16", "vqm"]
    ...
    for s in stems:
        for ext in order:
            asset = idx.resolve(s + "." + ext.upper(), ext, source=geo_asset)
            if not asset:
                asset = idx.resolve(s + "." + ext.lower(), ext, source=geo_asset)
            if not asset:
                asset = idx.resolve(s, ext, source=geo_asset)
            if asset:
                return asset, f"preferred_{ext}"

    return resolver.resolve_slot_as_direct_texture(raw_slot, norm, idx, geo_asset), "current_fallback"


def resolve_material_asset(resolver, idx, geo_asset, raw_slot: str, tmt_assets, map_assets, map_ref_stems, direct_map_prefer: str = "current", normalized_slot_swap: Optional[Dict[str, str]] = None):
    norm_original = resolver.normalize_geo_slot(raw_slot)
    norm, swap_evidence = apply_normalized_slot_swap(norm_original, normalized_slot_swap or {})
    ...
    if not resolved:
        resolved, pref_evidence = resolve_direct_texture_with_preference(
            resolver, raw_slot, norm, idx, geo_asset, direct_map_prefer
        )
        kind = "direct_texture"
        confidence = "medium"
        evidence = "GEO slot literal stem resolved directly"
        if pref_evidence != "current":
            evidence += f"; direct_map_prefer={direct_map_prefer}:{pref_evidence}"
...
def export_texture_for_asset(resolver, idx, texture_asset, tex_dir: Path, written: Dict[str, str], palette_rgb: Optional[bytes], vqm_layout: str) -> str:
    ...
    if texture_asset.ext == "m16":
        ...
        if cbk:
            parsed = decode_vqm_pixels(data, idx.payloads.get(cbk.asset_id, b""), vqm_layout)
            if parsed:
                image_name = write_indexed_image(out_base, parsed[0], parsed[1], parsed[2], palette_rgb)
...
def write_obj_mtl(vehicle_dir: Path, vehicle_name: str, part_rows: List[Tuple], export_textures: bool, resolver, idx, palette_rgb: Optional[bytes] = None, flat_color_mode: str = "auto", vqm_layout: str = "yx", direct_map_prefer: str = "current", textured_kd_mode: str = "white", uv_flip_u_slots: Optional[List[str]] = None, uv_flip_u_textures: Optional[List[str]] = None, uv_flip_u_materials: Optional[List[str]] = None, reverse_face_order_slots: Optional[List[str]] = None, reverse_face_order_textures: Optional[List[str]] = None, reverse_face_order_materials: Optional[List[str]] = None, normalized_slot_swap: Optional[Dict[str, str]] = None):
    uv_flip_u_slots = uv_flip_u_slots or []
    uv_flip_u_textures = uv_flip_u_textures or []
    uv_flip_u_materials = uv_flip_u_materials or []
    reverse_face_order_slots = reverse_face_order_slots or []
    reverse_face_order_textures = reverse_face_order_textures or []
    reverse_face_order_materials = reverse_face_order_materials or []
    normalized_slot_swap = normalized_slot_swap or {}
    ...
    with obj_path.open("w", encoding="utf-8") as obj:
        ...
        for item in part_rows:
            ...
            # Targeted U-flip overrides are diagnostic: they let us test whether
            # specific rear decal/material groups such as BKBK are mirrored due
            # to UV convention rather than texture decode.
            local_vt_indices: List[List[int]] = []
            local_uv_flip_u: List[bool] = []
            local_reverse_face_order: List[bool] = []
            for face in geo_mesh.faces:
                raw_slot_for_uv = face.texture_name or f"COLOR_{face.color[0]:02x}{face.color[1]:02x}{face.color[2]:02x}"
                resolved_for_uv, _kind_for_uv, _confidence_for_uv, _evidence_for_uv, norm_for_uv = resolve_material_asset(
                    resolver, idx, geo_asset, raw_slot_for_uv, tmt_assets, map_assets, map_ref_stems, direct_map_prefer, normalized_slot_swap
                )
                texture_asset_for_uv = None
                material_key_for_uv = f"UNRES_{safe_name(norm_for_uv or raw_slot_for_uv)}"
                if resolved_for_uv:
                    if resolved_for_uv.ext == "tmt":
                        texture_asset_for_uv = first_texture_from_tmt(resolver, idx, resolved_for_uv)
                        material_key_for_uv = f"TMT_{safe_name(resolved_for_uv.name)}"
                    else:
                        texture_asset_for_uv = resolved_for_uv
                        material_key_for_uv = f"TEX_{safe_name(resolved_for_uv.name)}"
                texture_name_for_uv = texture_asset_for_uv.name if texture_asset_for_uv else ""

                flip_u = should_flip_u_for_face(
                    raw_slot_for_uv, norm_for_uv, material_key_for_uv, texture_name_for_uv,
                    uv_flip_u_slots, uv_flip_u_textures, uv_flip_u_materials
                )
                reverse_face_order = should_reverse_face_order_for_face(
                    raw_slot_for_uv, norm_for_uv, material_key_for_uv, texture_name_for_uv,
                    reverse_face_order_slots, reverse_face_order_textures, reverse_face_order_materials
                )
                local_uv_flip_u.append(flip_u)
                local_reverse_face_order.append(reverse_face_order)

                vt_row = []
                for ref in face.refs:
                    u = 1.0 - ref.u if flip_u else ref.u
                    obj.write(f"vt {u:.8g} {1.0 - ref.v:.8g}\n")
            ...
            for face_idx, face in enumerate(geo_mesh.faces):
                ...
                resolved, kind, confidence, evidence, norm = resolve_material_asset(
                    resolver, idx, geo_asset, raw_slot, tmt_assets, map_assets, map_ref_stems, direct_map_prefer, normalized_slot_swap
                )
                texture_asset = None
                material_key = ""
                if resolved:
                ...
                if material_key not in materials:
                    geo_face_color = face.color
                    diffuse_color = geo_face_color
                    palette_index = None
                    diffuse_source = "geo_face_rgb_direct"

                    # For textured OBJ/MTL materials, Blender and most MTL importers
                    # multiply map_Kd by Kd. The original GEO face color is source
                    # evidence, but using it as active Kd causes incorrect tinting
                    # or darkening in Blender. Default to white Kd for textured
                    # materials and preserve GEO color separately in materials.tsv.
                    is_textured_material = texture_asset is not None
                    if is_textured_material:
                        if textured_kd_mode == "white":
                            ...
                            palette_index = nearest_palette_index_rgb(palette_rgb, geo_face_color)
                            pal_color = palette_rgb_at(palette_rgb, palette_index)
                            if pal_color is not None:
                                diffuse_color = pal_color
                                diffuse_source = "textured_palette_nearest_to_geo_face_rgb"
                            else:
                                diffuse_source = "textured_geo_face_rgb_direct"
                        else:
                            diffuse_source = "textured_geo_face_rgb_direct"

                    # Runtime evidence indicates flat GEO face colors are quantized to the
                    # nearest global 8bpp palette index during geometry construction:
                    #   section +0x09/+0x0a/+0x0b = raw face RGB bytes
                    #   section +0x08 = cache_nearest_palette_cube_index(r,g,b)
                    #   render record +0x1c = section +0x08
                    # Therefore untextured COLOR_* materials should preview using that
                    # nearest palette entry when a palette is available.
                    elif raw_slot.startswith("COLOR_") and (flat_color_mode in {"auto", "palette-nearest"}):
                        palette_index = nearest_palette_index_rgb(palette_rgb, geo_face_color)
                        pal_color = palette_rgb_at(palette_rgb, palette_index)
                        if pal_color is not None:
                            diffuse_color = pal_color
                            diffuse_source = "palette_nearest_to_geo_face_rgb"
                    elif raw_slot.startswith("COLOR_"):
                        diffuse_source = "geo_face_rgb_direct"
                    ...
                    if export_textures and texture_asset:
                        mat["map_kd"] = export_texture_for_asset(resolver, idx, texture_asset, tex_dir, texture_written, palette_rgb, vqm_layout)
                    materials[material_key] = mat

                obj.write(f"usemtl {material_key}\n")
                refs = []
                face_corner_order = list(range(len(face.refs)))
                if local_reverse_face_order[face_idx]:
                    face_corner_order = list(reversed(face_corner_order))
                for out_j in face_corner_order:
                    ref = face.refs[out_j]
                    ...
                    vti = local_vt_indices[face_idx][out_j]
                    refs.append(f"{vi}/{vti}/{ni}")
                # Keep original winding by default. Targeted diagnostics can reverse
                # face-corner order for suspected mirrored decal groups.
                ...
                face_report.append({
                    ...
                    "confidence": confidence if resolved else "none",
                    "uv_flip_u": "1" if local_uv_flip_u[face_idx] else "0",
                    "reverse_face_order": "1" if local_reverse_face_order[face_idx] else "0",
                    "evidence": (evidence if resolved else "unresolved") + ("; uv_flip_u_override" if local_uv_flip_u[face_idx] else "") + ("; reverse_face_order_override" if local_reverse_face_order[face_idx] else ""),
                })
    ...
    with mtl_path.open("w", encoding="utf-8") as mtl:
        ...
        for name, mat in materials.items():
            r, g, b = mat.get("diffuse_color", mat["color"])
            mtl.write(f"\nnewmtl {name}\n")
...
def is_shadow_object_name(name: str) -> bool:
    """Return true for vehicle SOR/shadow-object geometry names.

    Asset-side evidence shows *_SOR GEO/object names are the dominant source of
    the black planes around exported vehicles. These are render auxiliary/shadow
    planes, not collision geometry. Keep the test conservative: only classify the
    explicit _SOR convention.
    """
    n = (name or "").upper()
    stem = n.rsplit(".", 1)[0]
    return stem.endswith("_SOR") or "_SOR_" in stem
...
def main() -> int:
    ...
    ap.add_argument("--flat-color-mode", choices=["auto", "direct", "palette-nearest"], default="auto", help="How to emit untextured COLOR_* material Kd values. auto uses palette-nearest when --palette-file is provided, otherwise direct.")
    ap.add_argument("--vqm-layout", choices=["yx", "yx_flipx", "yx_flipy", "yx_flipx_flipy", "xy", "xy_flipx", "xy_flipy", "xy_flipx_flipy"], default="yx", help="Intra-tile CBK layout for VQM decoding. Default yx based on decode-variant validation; xy preserves the older Open76-inspired behavior.")
    ap.add_argument("--direct-map-prefer", choices=["current", "vqm", "m16"], default="current", help="For literal direct .MAP slots where same-basename M16/VQM siblings exist, prefer the selected texture type. current preserves v15 behavior.")
    ap.add_argument("--textured-kd-mode", choices=["white", "geo-direct", "palette-nearest"], default="white", help="Diffuse Kd for textured OBJ/MTL materials. white avoids Blender tinting; geo-direct preserves older GEO face RGB; palette-nearest maps GEO face RGB through the active palette.")
    ap.add_argument("--uv-flip-u-slot", action="append", default=[], help="Diagnostic: flip U for faces whose raw/normalized GEO slot contains this pattern. Repeatable; comma-separated accepted.")
    ap.add_argument("--uv-flip-u-texture", action="append", default=[], help="Diagnostic: flip U for faces whose resolved texture filename contains this pattern. Repeatable; comma-separated accepted.")
    ap.add_argument("--uv-flip-u-material", action="append", default=[], help="Diagnostic: flip U for faces whose material name contains this pattern. Repeatable; comma-separated accepted.")
    ap.add_argument("--reverse-face-order-slot", action="append", default=[], help="Diagnostic: reverse OBJ face corner order for faces whose raw/normalized GEO slot contains this pattern.")
    ap.add_argument("--reverse-face-order-texture", action="append", default=[], help="Diagnostic: reverse OBJ face corner order for faces whose resolved texture filename contains this pattern.")
    ap.add_argument("--reverse-face-order-material", action="append", default=[], help="Diagnostic: reverse OBJ face corner order for faces whose material name contains this pattern.")
    ap.add_argument("--swap-normalized-slot", action="append", default=[], help="Diagnostic: swap normalized material slot before TMT/MAP resolution, e.g. BKLT:BKRT. Repeatable; comma-separated accepted.")
    ap.add_argument("--vdf-sobj-mode", choices=["skip", "separate", "include"], default="separate", help="How to handle VDF SOBJ auxiliary geometry. separate exports it outside the main vehicle OBJ; skip omits it; include preserves legacy identity-root inclusion.")
    ap.add_argument("--shadow-object-mode", choices=["include", "skip"], default="include", help="Deprecated compatibility alias for *_SOR handling. If set to skip and --vdf-sobj-mode is not explicitly changed, SOBJ/_SOR geometry is skipped.")
    ...
    args.uv_flip_u_slot = split_csv_patterns(args.uv_flip_u_slot)
    args.uv_flip_u_texture = split_csv_patterns(args.uv_flip_u_texture)
    args.uv_flip_u_material = split_csv_patterns(args.uv_flip_u_material)
    args.reverse_face_order_slot = split_csv_patterns(args.reverse_face_order_slot)
    args.reverse_face_order_texture = split_csv_patterns(args.reverse_face_order_texture)
    args.reverse_face_order_material = split_csv_patterns(args.reverse_face_order_material)
    args.normalized_slot_swap = parse_slot_swap_specs(args.swap_normalized_slot)
    ...
    # Backward compatibility with v12 command lines: --shadow-object-mode skip
    # means skip the VDF SOBJ/_SOR auxiliary unless the caller explicitly uses
    # the new --vdf-sobj-mode flag.
    if args.shadow_object_mode == "skip" and "--vdf-sobj-mode" not in sys.argv:
        vdf_sobj_mode = "skip"
    ...
    for vcf in vcf_assets:
        ...
        skipped_sobj_parts = 0
        skipped_sobj_faces = 0
        skipped_sobj_names = []
        separated_sobj_parts = 0
        separated_sobj_faces = 0
        separated_sobj_names = []
        for part_transform in transforms:
            ...
            try:
                mesh = parse_geo(data)
                row_tuple = (geo_asset, mesh, part_transform, tmt_assets, map_assets, map_ref_stems, vdf.name, vtf.name)
                is_vdf_sobj_aux = (part_transform.source == "SOBJ") or is_shadow_object_name(part_transform.name) or is_shadow_object_name(geo_asset.name) or is_shadow_object_name(mesh.name)
                if is_vdf_sobj_aux:
                    if vdf_sobj_mode == "skip":
                        skipped_sobj_parts += 1
                        skipped_sobj_faces += len(mesh.faces)
                        if len(skipped_sobj_names) < 80:
                            skipped_sobj_names.append(part_transform.name)
                        continue
                    if vdf_sobj_mode == "separate":
                        separated_sobj_parts += 1
                        separated_sobj_faces += len(mesh.faces)
                        if len(separated_sobj_names) < 80:
                            separated_sobj_names.append(part_transform.name)
                        sobj_aux_rows.append(row_tuple)
                        continue
                part_rows.append(row_tuple)
            except Exception as e:
        ...
        sobj_mtl_rel = ""
        if sobj_aux_rows:
            flat_color_mode_for_aux = args.flat_color_mode
            if flat_color_mode_for_aux == "auto" and palette_rgb is None:
                flat_color_mode_for_aux = "direct"
            aux_dir = vdir / "sobj_auxiliary"
            aux_dir.mkdir(parents=True, exist_ok=True)
            aux_name = safe_name(vcf.name).rsplit(".", 1)[0] + "_vdf_sobj_auxiliary"
            sobj_obj_path, sobj_mtl_path, sobj_materials, sobj_report = write_obj_mtl(
                aux_dir, aux_name, sobj_aux_rows, args.export_textures, resolver, idx, palette_rgb, flat_color_mode_for_aux, args.vqm_layout, args.direct_map_prefer, args.textured_kd_mode,
                args.uv_flip_u_slot, args.uv_flip_u_texture, args.uv_flip_u_material,
                args.reverse_face_order_slot, args.reverse_face_order_texture, args.reverse_face_order_material,
                args.normalized_slot_swap
            )
            # Mark auxiliary rows explicitly in the report stream while keeping them out of the main vehicle OBJ.
            for rr in sobj_report:
                rr["evidence"] = str(rr.get("evidence", "")) + "; source=VDF_SOBJ_auxiliary"
            face_rows.extend(sobj_report)
            for mname, mat in sobj_materials.items():
                material_rows.append({"vehicle": vcf.name, "material": mname, "auxiliary": "vdf_sobj", **mat})
            sobj_obj_rel = sobj_obj_path.relative_to(args.out).as_posix()
            sobj_mtl_rel = sobj_mtl_path.relative_to(args.out).as_posix()

        if part_rows:
            flat_color_mode = args.flat_color_mode
            if flat_color_mode == "auto" and palette_rgb is None:
                flat_color_mode = "direct"
            obj_path, mtl_path, materials, report = write_obj_mtl(vdir, vcf.name, part_rows, args.export_textures, resolver, idx, palette_rgb, flat_color_mode, args.vqm_layout, args.direct_map_prefer, args.textured_kd_mode, args.uv_flip_u_slot, args.uv_flip_u_texture, args.uv_flip_u_material, args.reverse_face_order_slot, args.reverse_face_order_texture, args.reverse_face_order_material, args.normalized_slot_swap)
            ...
            vehicle_rows.append({
                ...
                "skipped_sobj_parts": skipped_sobj_parts,
                "skipped_sobj_faces": skipped_sobj_faces,
                "skipped_sobj_names": ";".join(skipped_sobj_names),
                "separated_sobj_parts": separated_sobj_parts,
                "separated_sobj_faces": separated_sobj_faces,
                "separated_sobj_names": ";".join(separated_sobj_names),
                "sobj_obj": sobj_obj_rel,
                "sobj_mtl": sobj_mtl_rel,
            ...
            vehicle_rows.append({"vehicle": vcf.name, "status": "no_geo_parts", "vdf": vdf.name, "vtf": vtf.name, "parts": 0, "faces": 0, "materials": 0, "skipped_sobj_parts": skipped_sobj_parts, "skipped_sobj_faces": skipped_sobj_faces, "skipped_sobj_names": ";".join(skipped_sobj_names),
                "separated_sobj_parts": separated_sobj_parts,
                "separated_sobj_faces": separated_sobj_faces,
                "separated_sobj_names": ";".join(separated_sobj_names),
                "sobj_obj": sobj_obj_rel,
                "sobj_mtl": sobj_mtl_rel, "alternate_state_note_count": len(alternate_state_notes), "alternate_state_notes": "; ".join(alternate_state_notes[:60]), "parse_errors": "; ".join(parse_errors[:20])})

    write_tsv(args.out / "vehicle_export_summary.tsv", vehicle_rows, ["vehicle", "status", "vdf", "vtf", "parts", "faces", "materials", "skipped_sobj_parts", "skipped_sobj_faces", "skipped_sobj_names", "separated_sobj_parts", "separated_sobj_faces", "separated_sobj_names", "obj", "mtl", "sobj_obj", "sobj_mtl", "alternate_state_note_count", "alternate_state_notes", "parse_errors"])
    if face_rows:
        write_tsv(args.out / "face_material_resolution.tsv", face_rows, ["vehicle", "vdf", "vtf", "geo", "part", "parent", "face_index", "raw_slot", "normalized_slot", "material", "resolved_kind", "resolved_name", "texture_asset", "confidence", "uv_flip_u", "reverse_face_order", "evidence"])
    if material_rows:
        write_tsv(args.out / "materials.tsv", material_rows, ["vehicle", "material", "auxiliary", "name", "raw_slot", "normalized_slot", "resolved_kind", "resolved_name", "resolved_asset_id", "texture_asset_name", "texture_asset_id", "confidence", "evidence", "color", "geo_face_color", "diffuse_color", "diffuse_source", "palette_index", "map_kd"])
    ...
    summary = {
        ...
        "uv_flip_u_slot": args.uv_flip_u_slot,
        "uv_flip_u_texture": args.uv_flip_u_texture,
        "uv_flip_u_material": args.uv_flip_u_material,
        "reverse_face_order_slot": args.reverse_face_order_slot,
        "reverse_face_order_texture": args.reverse_face_order_texture,
        "reverse_face_order_material": args.reverse_face_order_material,
        "normalized_slot_swap": args.normalized_slot_swap,
        "vdf_sobj_mode": vdf_sobj_mode,
        "shadow_object_mode_compat": args.shadow_object_mode,
        "skipped_sobj_parts": sum(int(r.get("skipped_sobj_parts", 0) or 0) for r in vehicle_rows),
        "skipped_sobj_faces": sum(int(r.get("skipped_sobj_faces", 0) or 0) for r in vehicle_rows),
        "separated_sobj_parts": sum(int(r.get("separated_sobj_parts", 0) or 0) for r in vehicle_rows),
        "separated_sobj_faces": sum(int(r.get("separated_sobj_faces", 0) or 0) for r in vehicle_rows),
        "palette_file": str(args.palette_file) if args.palette_file else "",
        ...
        "notes": "VDF VGEO state-0 hierarchy is applied; alternate damage GEOs are bound by VGEO slot index; VTF refs use exact plus leading-prefix fallback; VQM decoding defaults to yx intra-tile layout; direct MAP preference is configurable.",
    }
    (args.out / "summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True), encoding="utf-8")
    (args.out / "README.txt").write_text("""I76 vehicle GEO OBJ exporter v20
...
Outputs:
  ...
  <vehicle>/sobj_auxiliary/* when --vdf-sobj-mode separate and VDF SOBJ geometry is present
...
Important:
  ...
  * VTF/TMT texture list interpretation follows Open76 VtfParser:
    ...
    8-byte strings.
  * Parts are transformed through VDF VGEO hierarchy.
  ...
  * VQM decoding defaults to the validated yx intra-tile layout. Use
    --vqm-layout to compare alternate interpretations.
  * --direct-map-prefer can compare direct literal .MAP sibling resolution,
    e.g. preferring VQM over M16 for PP11BKU1.MAP-style slots.
  * --textured-kd-mode controls OBJ/MTL Kd for textured materials. Default
    white prevents Blender from multiplying/tinting texture maps.
  * --uv-flip-u-slot/texture/material are targeted diagnostics for mirrored
    decals. They do not change global UV convention.
  * --reverse-face-order-slot/texture/material are targeted diagnostics for
    UV-corner/winding issues where a full U-flip mirrors the whole texture island.
  * --swap-normalized-slot is a resolver-layer diagnostic for left/right slot
    convention issues, e.g. BKLT:BKRT,BKRT:BKLT.
""", encoding="utf-8")
