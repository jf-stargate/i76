#!/usr/bin/env python3
"""
i76_vehicle_geo_obj_export_v18.py
...
Usage:
  python3 i76_vehicle_geo_obj_export_v18.py \
      --zfs I76.ZFS \
      --zix I76.ZIX \
...
Export all vehicles with VDF/VTF:
  python3 i76_vehicle_geo_obj_export_v18.py --zfs I76.ZFS --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
        ...
        for item in part_rows:
            ...
            for face_idx, face in enumerate(geo_mesh.faces):
                ...
                face_report.append({
                    ...
                    "evidence": (evidence if resolved else "unresolved") + ("; uv_flip_u_override" if local_uv_flip_u[face_idx] else ""),
                })
...
def main() -> int:
    ...
    for vcf in vcf_assets:
        ...
        if part_rows:
            ...
            obj_path, mtl_path, materials, report = write_obj_mtl(vdir, vcf.name, part_rows, args.export_textures, resolver, idx, palette_rgb, flat_color_mode, args.vqm_layout, args.direct_map_prefer, args.textured_kd_mode, args.uv_flip_u_slot, args.uv_flip_u_texture, args.uv_flip_u_material)
    ...
    if face_rows:
        write_tsv(args.out / "face_material_resolution.tsv", face_rows, ["vehicle", "vdf", "vtf", "geo", "part", "parent", "face_index", "raw_slot", "normalized_slot", "material", "resolved_kind", "resolved_name", "texture_asset", "confidence", "uv_flip_u", "evidence"])
    if material_rows:
    ...
    (args.out / "README.txt").write_text("""I76 vehicle GEO OBJ exporter v18
...
Important:
  ...
  * --uv-flip-u-slot/texture/material are targeted diagnostics for mirrored
    decals such as rear BKBK text. They do not change global UV convention.
""", encoding="utf-8")
