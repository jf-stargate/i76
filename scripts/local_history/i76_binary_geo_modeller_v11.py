...
def write_indexed_image_rgba_open76(path_no_ext: Path, width: int, height: int, pixels: bytes,
    ...
    """
    has_alpha = (texture_alpha_mode in {"index-ff", "transparent-flags"}) and alpha_enabled_for_material and any(px == 0xff for px in pixels)
...
def export_texture_for_asset_oriented(helper, resolver, idx, texture_asset, tex_dir: Path, written: Dict[str, Dict[str, object]],
    ...
    if alpha_enabled_for_material and texture_alpha_mode in {"index-ff", "transparent-flags"}:
        out_name += "_alpha"
...
def add_material(helper, resolver, idx, tex_dir: Path, materials: Dict[str, Dict[str, object]], mat_info,
    ...
    if export_textures and texture_asset:
        alpha_enabled_for_material = bool(mat_info.get("texture_transparency_enabled", False)) if texture_alpha_mode == "transparent-flags" else (texture_alpha_mode == "index-ff")
...
def main() -> int:
    ...
    ap.add_argument("--texture-alpha-mode", choices=["transparent-flags", "index-ff", "opaque"], default="transparent-flags", help="PNG alpha export policy. transparent-flags maps palette index 0xff to alpha only for textured faces whose GEO SurfaceFlags2 is 5 or 7; index-ff applies alpha globally to every texture using palette index 0xff; opaque preserves fully opaque RGB output.")
