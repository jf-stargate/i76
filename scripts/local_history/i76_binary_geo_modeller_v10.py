...
def write_indexed_image_rgba_open76(path_no_ext: Path, width: int, height: int, pixels: bytes,
                                   palette_rgb: Optional[bytes], texture_alpha_mode: str) -> Tuple[str, bool]:
    ...
    """
    has_alpha = texture_alpha_mode == "index-ff" and any(px == 0xff for px in pixels)
...
def export_texture_for_asset_oriented(helper, resolver, idx, texture_asset, tex_dir: Path, written: Dict[str, Dict[str, object]],
                                      ...
                                      texture_alpha_mode: str) -> Dict[str, object]:
    ...
    """
    cache_key = texture_asset.asset_id + "|image_transform=" + texture_image_transform + "|vqm_layout=" + vqm_layout + "|alpha=" + texture_alpha_mode
    ...
    if texture_asset.ext == "m16":
        ...
        if parsed:
            ...
            image_name, has_alpha = write_indexed_image_rgba_open76(out_base, w, h, pix, palette_rgb, texture_alpha_mode)
...
def add_material(helper, resolver, idx, tex_dir: Path, materials: Dict[str, Dict[str, object]], mat_info,
    ...
    if export_textures and texture_asset:
        tex_meta = export_texture_for_asset_oriented(helper, resolver, idx, texture_asset, tex_dir, texture_written, palette_rgb, vqm_layout, texture_image_transform, texture_alpha_mode)
...
def main() -> int:
    ...
    ap.add_argument("--texture-alpha-mode", choices=["index-ff", "opaque"], default="index-ff", help="PNG alpha export policy. index-ff maps palette index 0xff to transparent alpha, matching Open76-style transparent texture handling; opaque preserves fully opaque RGB output.")
...
Material-color policy:
  ...
  V9 exports palette index 0xff as PNG alpha for M16/VQM textures and writes blender_apply_i76_material_settings.py to set alpha clip / closest interpolation after OBJ import.
