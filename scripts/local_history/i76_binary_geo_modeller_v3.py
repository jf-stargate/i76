#!/usr/bin/env python3
"""
i76_binary_geo_modeller_v3.py

Third-pass Interstate '76 binary-faithful vehicle GEO modeller with explicit OBJ/raster and handedness orientation probes.
...
def geometry_transform_determinant_sign(mode: str) -> int:
    if mode in {"identity", "blender-xzy-rh"}:
        return 1
    if mode in {"mirror-x", "mirror-y", "mirror-z", "blender-xzy"}:
        return -1
    return 1
...
def main() -> int:
    ...
    ap.add_argument("--obj-geometry-transform", choices=["identity", "mirror-x", "mirror-y", "mirror-z", "blender-xzy", "blender-xzy-rh"], default="identity", help="Final OBJ-view coordinate transform. Use mirror-* to test missing handedness conversion without changing UVs/textures.")
...
Orientation probes:
  ...
  --obj-geometry-transform identity|mirror-x|mirror-y|mirror-z|blender-xzy|blender-xzy-rh changes only the final OBJ coordinate view and is intended to test missing handedness/axis conversion.
""", encoding="utf-8")
