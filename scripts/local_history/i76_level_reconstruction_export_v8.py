#!/usr/bin/env python3
"""
Interstate '76 level reconstruction exporter, v8.
...
Baseline intent:
  ...
  - Add first-pass SDF child candidate resolution against real .geo resources.
...
def sdf_child_candidates(sdf_path: Path, file_index: Dict[str, List[Path]], max_rows: int = 512) -> List[dict]:
    try:
        data = sdf_path.read_bytes()
    except Exception as e:
        return [{"sdf_path": str(sdf_path), "offset_hex": "", "token": "", "geo_name": "", "geo_found": False, "score": 0, "reasons": f"read_error:{e}"}]
    rows: List[dict] = []
    seen = set()
    for off, tok in extract_ascii_tokens(data):
        ...
        rows.append({
            ...
            "geo_name": geo_name,
            "geo_found": bool(hit),
            "geo_path": str(hit) if hit else "",
            "score": score,
            "reasons": reasons,
        })
    rows.sort(key=lambda r: (-int(r["score"]), r["geo_name"], r["offset_hex"]))
...
def build_resolution_rows(mission_stem: str, mission_path: Path, records: List[dict], file_index: Dict[str, List[Path]], parent_tag: str) -> Tuple[List[dict], List[dict], List[dict]]:
    res_rows: List[dict] = []
    sdf_rows: List[dict] = []
    ...
    for r in records:
        ...
        if hit and cls["kind"] == "sdf":
            ...
            if key not in sdf_cache:
                sdf_cache[key] = sdf_child_candidates(hit, file_index)
...
def export(args: argparse.Namespace) -> dict:
    ...
    out_dir = timestamped_out_dir(args.out_dir, "i76_level_reconstruction_export_v8")
    ...
    manifest = {
        ...
        "runtime_approximate": [
            ...
            "SDF child candidate extraction is token/resource-index based until exact descriptor table parser is implemented",
            "GEO geometry instantiation is not yet wired into this revision",
        ],
        "missions": [],
        "warnings": [],
    }
    ...
    write_tsv(
        ...
        all_sdf_rows,
        ["mission", "object_label", "object_index", "object_class_hex", "sdf_path", "offset_hex", "token", "geo_name", "geo_found", "geo_path", "score", "reasons"],
    )
    write_tsv(
    ...
    notes = [
        "i76_level_reconstruction_export_v8 notes",
        "",
        ...
        "  tables/sdf_child_candidates.tsv         first-pass SDF -> GEO candidate links",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--copy-resolved", action="store_true", help="Copy resolved root SDF/VCF and candidate GEO files into output folders")
