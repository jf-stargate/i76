#!/usr/bin/env python3
"""
Interstate '76 level reconstruction exporter, v12.
...
def build_embedded_asset_index(args, manifest_warnings: Optional[List[str]] = None) -> EmbeddedAssetIndex:
    ...
    try:
        ...
        return EmbeddedAssetIndex(names, warnings)
...
def sdf_child_candidates(sdf_path: Path, file_index: Dict[str, List[Path]], embedded_index: Optional[EmbeddedAssetIndex] = None, max_rows: int = 2048) -> List[dict]:
    # Strict SGEO parsing is the primary v9 path.  It replaces the v8 token scan
    # for normal SDFs and follows the earlier Open76-aligned probe.
    rows = parse_sdf_sgeo_parts(sdf_path, file_index, embedded_index=embedded_index, include_extra_records=True)
    if rows:
...
def build_resolution_rows(mission_stem: str, mission_path: Path, records: List[dict], file_index: Dict[str, List[Path]], embedded_index: Optional[EmbeddedAssetIndex], parent_tag: str) -> Tuple[List[dict], List[dict], List[dict]]:
    res_rows: List[dict] = []
    sdf_rows: List[dict] = []
    ...
    for r in records:
        ...
        if hit and cls["kind"] == "sdf":
            ...
            if key not in sdf_cache:
                sdf_cache[key] = sdf_child_candidates(hit, file_index, embedded_index=embedded_index)
            for c in sdf_cache[key]:
                ...
                sdf_rows.append(c2)
    return res_rows, sdf_rows, unresolved


def export(args: argparse.Namespace) -> dict:
    input_path = Path(args.input)
    out_dir = timestamped_out_dir(args.out_dir, "i76_level_reconstruction_export_v12")
    ...
    manifest = {
        ...
        "runtime_approximate": [
            ...
            "GEO geometry instantiation is not yet wired into this revision; v12 resolves embedded GEO names when the resolver is available but does not extract meshes",
        ],
        "missions": [],
        "warnings": [],
    }
    ...
    for msn in sorted(missions):
        try:
            ...
            for parent_tag, recs in (("ODEF", odef_records), ("LDEF", ldef_records)):
                ...
                res_rows, sdf_rows, unresolved = build_resolution_rows(msn.stem.lower(), msn, recs, file_index, embedded_index, parent_tag)
            ...
            if args.copy_resolved:
                ...
                for row in all_sdf_rows:
                    gp = row.get("geo_path")
                    if gp and gp not in copied_geo:
    ...
    write_tsv(
        ...
        all_sdf_rows,
        ["mission", "object_label", "object_index", "object_class_hex", "sdf_path", "sdf_object_name", "sgeo_chunk_offset_hex", "declared_part_count", "offset_hex", "parser", "part_kind", "part_index", "token", "parent_name", "geo_name", "geo_found", "geo_source", "geo_path", "score", "reasons", "right_x", "right_y", "right_z", "up_x", "up_y", "up_z", "forward_x", "forward_y", "forward_z", "pos_x", "pos_y", "pos_z"],
    )
    write_tsv(
    ...
    notes = [
        "i76_level_reconstruction_export_v12 notes",
        "",
        "This revision is based on the repo/local_history v4 BWD2 mission-parser workflow.",
        ...
        "",
        "SDF child extraction is still a candidate probe.  Use Ghidra/SDF descriptor evidence to replace it with",
        "a strict SDFC/SOBJ/SGEO/SCHK/LOBJ parser once the exact child-table shape is pinned down.",
    ]
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Interstate '76 level reconstruction exporter with v42 class-specific object resource resolution")
    ...
    if not (args.write_terrain or args.write_adaptive_overlay or args.write_odef_axes or args.write_rseg or args.write_resource_markers):
