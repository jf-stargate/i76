#!/usr/bin/env python3
"""
i76_vehicle_export_all_damage_states_v2.py

Wrapper/validator for the existing Interstate '76 vehicle OBJ exporter.

Purpose:
  - Run i76_vehicle_geo_obj_export_v7.py, v8.py, or another compatible exporter for damage states 0, 1, 2, and 3.
...
Example:
  python3 scripts/i76_vehicle_export_all_damage_states_v2.py \
    --exporter scripts/i76_vehicle_geo_obj_export_v7.py \
    --zfs orig/I76.ZFS \
    --zix orig/I76.ZIX \
...
Notes:
  - This script assumes the selected exporter accepts --damage-state 0|1|2|3.
  ...
  - It parses generated OBJ/MTL files directly, so it remains useful even if
    exporter report filenames change.
"""
...
def choose_exporter(args: argparse.Namespace) -> Path:
    if args.exporter:
        candidate = Path(args.exporter)
        if not candidate.exists():
            raise FileNotFoundError(
                f"Exporter not found: {candidate}\n"
                f"Use --exporter scripts/i76_vehicle_geo_obj_export_v7.py if v7 is your latest local exporter."
            )
        return candidate

    candidates = [
        Path("scripts/i76_vehicle_geo_obj_export_v8.py"),
        Path("scripts/i76_vehicle_geo_obj_export_v7.py"),
    ]
    for candidate in candidates:
        if candidate.exists():
            eprint(f"[auto] selected exporter: {candidate}")
            return candidate

    raise FileNotFoundError(
        "No exporter specified and neither scripts/i76_vehicle_geo_obj_export_v8.py "
        "nor scripts/i76_vehicle_geo_obj_export_v7.py exists. Pass --exporter explicitly."
    )


def require_existing_file(label: str, path_text: str) -> Path:
    path = Path(path_text)
    if not path.exists():
...
def run_exporter_for_state(args: argparse.Namespace, exporter_path: Path, damage_state: int) -> Path:
    ...
    if not args.skip_export:
        try:
            ...
            raise RuntimeError(
                f"Exporter failed for damage_state={damage_state} with exit code {ex.returncode}.\n"
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--exporter", default="", help="Path to compatible vehicle OBJ exporter. If omitted, tries v8 then v7 under ./scripts")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    exporter_path = choose_exporter(args)
    require_existing_file("ZFS archive", args.zfs)
    require_existing_file("ZIX index", args.zix)
    require_existing_file("texture resolver", args.resolver)

    for d in DAMAGE_STATES:
        state_roots[d] = run_exporter_for_state(args, exporter_path, d)
