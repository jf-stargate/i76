#!/usr/bin/env python3
"""
i76_vehicle_palette_candidate_probe_v1.py
...
Inputs:
  ...
  --out scratch_data/i76_vehicle_palette_candidates_v1
...
def contact_sheet_for_texture(
    ...
    try:
        ...
        for i, pal in enumerate(use):
            ...
            sheet.paste(img, (x, y))
            label = pal.key[:38]
            draw.text((x + 2, y + thumb_h + 2), label, fill=(230, 230, 230))
        sheet_path = sheets_dir / (safe_name(tex_name) + "_palette_contact_sheet.png")
        sheets_dir.mkdir(parents=True, exist_ok=True)
        sheet.save(sheet_path)
    except Exception:
        # Individual renders still exist.
        pass
...
def main() -> int:
    ...
    ap.add_argument("--max-palettes", type=int, default=0, help="0 = all palettes")
    args = ap.parse_args()
    ...
    palettes = extract_act_candidates(idx, args.out)
    palettes.extend(load_extra_palettes(args.extra_palette, args.out))
    ...
    for asset in assets:
        ...
        rows = contact_sheet_for_texture(args.out, asset.name, w, h, pix, palettes, args.max_palettes)
