#!/usr/bin/env python3
"""
i76_vehicle_geo_obj_export_v16.py
...
Usage:
  python3 i76_vehicle_geo_obj_export_v16.py \
      --zfs I76.ZFS \
      --zix I76.ZIX \
...
Export all vehicles with VDF/VTF:
  python3 i76_vehicle_geo_obj_export_v16.py --zfs I76.ZFS --zix I76.ZIX \
      --resolver i76_vehicle_texture_slot_resolver_v5.py \
...
def write_obj_mtl(vehicle_dir: Path, vehicle_name: str, part_rows: List[Tuple], export_textures: bool, resolver, idx, palette_rgb: Optional[bytes] = None, flat_color_mode: str = "auto", vqm_layout: str = "yx", direct_map_prefer: str = "current"):
        ...
        for item in part_rows:
            ...
            for face_idx, face in enumerate(geo_mesh.faces):
                ...
                if material_key not in materials:
                    ...
                    if raw_slot.startswith("COLOR_") and (flat_color_mode in {"auto", "palette-nearest"}):
                        palette_index = nearest_palette_index_rgb(palette_rgb, face.color)
...
def main() -> int:
    ...
    for vcf in vcf_assets:
        ...
        if sobj_aux_rows:
            ...
            sobj_obj_path, sobj_mtl_path, sobj_materials, sobj_report = write_obj_mtl(
                aux_dir, aux_name, sobj_aux_rows, args.export_textures, resolver, idx, palette_rgb, flat_color_mode_for_aux, args.vqm_layout, args.direct_map_prefer
            )
        ...
        if part_rows:
            ...
            obj_path, mtl_path, materials, report = write_obj_mtl(vdir, vcf.name, part_rows, args.export_textures, resolver, idx, palette_rgb, flat_color_mode, args.vqm_layout, args.direct_map_prefer)
    ...
    if material_rows:
        write_tsv(args.out / "materials.tsv", material_rows, ["vehicle", "material", "auxiliary", "name", "raw_slot", "normalized_slot", "resolved_kind", "resolved_name", "resolved_asset_id", "texture_asset_name", "texture_asset_id", "confidence", "evidence", "color", "diffuse_color", "diffuse_source", "palette_index", "map_kd"])
    ...
    (args.out / "README.txt").write_text("""I76 vehicle GEO OBJ exporter v16
