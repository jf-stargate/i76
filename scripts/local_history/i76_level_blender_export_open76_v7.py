#!/usr/bin/env python3
"""
Interstate '76 Open76-aligned terrain OBJ exporter, v7.
...
v7 exports a welded global mesh and adds seam-height modes:
  - neighbor: Open76-style neighbor edge sample.
  - clamp-own: use current patch last row/column for seam edge.
  - average: average current edge and neighbor edge heights to reduce visible ridges.
...
def stitched_sample(
    ...
    def combine(a_word: int, b_word: int, mode: str) -> int:
        if mode == "neighbor":
            return b_word
        if mode == "clamp-own":
            return a_word
        if mode == "average":
            # Average the full packed word only for height bits; preserve the neighbor surface class.
            ah = a_word & 0x0FFF
            bh = b_word & 0x0FFF
            avg_h = int(round((ah + bh) * 0.5)) & 0x0FFF
            return (b_word & 0xF000) | avg_h
        raise ValueError(f"unknown edge mode: {mode}")

    if local_x == PAGE_W and local_z < PAGE_H:
        own_word = sample_page(pages, own, PAGE_W - 1, local_z)
        right = zmap.get(cell_x + 1, cell_z)
        if right != 0xFF:
            nei_word = sample_page(pages, right, 0, local_z)
            return combine(own_word, nei_word, edge_mode)

    if local_z == PAGE_H and local_x < PAGE_W:
        own_word = sample_page(pages, own, local_x, PAGE_H - 1)
        bottom = zmap.get(cell_x, cell_z + 1)
        if bottom != 0xFF:
            nei_word = sample_page(pages, bottom, local_x, 0)
            return combine(own_word, nei_word, edge_mode)

    if local_x == PAGE_W and local_z == PAGE_H:
        own_word = sample_page(pages, own, PAGE_W - 1, PAGE_H - 1)
        br = zmap.get(cell_x + 1, cell_z + 1)
        if br != 0xFF:
            nei_word = sample_page(pages, br, 0, 0)
            return combine(own_word, nei_word, edge_mode)

    # Missing-neighbor behavior for outer map borders.
    if edge_mode == "zero":
        return 0
...
def write_mission_obj(
    ...
    *,
    ...
    with obj_path.open("w", encoding="utf-8") as f:
        f.write("# Interstate '76 Open76-aligned welded terrain OBJ v7\n")
...
def export(input_path: Path, out_dir: Path, mission_filter: str, origin_mode: str, obj_coordinates: str, edge_mode: str, vertical_scale: float, write_normals: bool, fail_empty: bool) -> dict:
    ...
    manifest_path = out_dir / "i76_open76_welded_terrain_obj_manifest_v7.json"
...
def main() -> int:
    ...
    ap.add_argument("--edge-mode", choices=["neighbor", "clamp-own", "average", "zero"], default="average")
