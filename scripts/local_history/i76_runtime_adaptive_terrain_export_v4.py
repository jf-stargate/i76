#!/usr/bin/env python3
"""
Interstate '76 runtime-shaped adaptive terrain OBJ exporter, v2.
...
@dataclasses.dataclass
class Vertex:
    # sx/sz are the TER/runtime lookup sample coordinates.
    # wx/wz are the logical terrain coordinates used for OBJ/world placement.
    # They differ only when boundary sampling clamps an outer active page edge
    # to the page's final valid sample while preserving the 128-sample patch span.
    sx: int
    sz: int
    wx: int
    wz: int
...
class AdaptiveBuilder:
    def __init__(self, terrain: RuntimeTerrain, args: argparse.Namespace):
        ...
        self.active_cells = {(cx, cz) for cx, cz, _ in self.t.zmap.used_cells()}
        self.split_origin_x, self.split_origin_z = self.compute_split_origin_world()

    def compute_split_origin_world(self) -> Tuple[float, float]:
        # v1-v3 bug: the OBJ was centered, but the split-distance heuristic
        # used raw runtime sample coordinates.  For M06 the active cells are near
        # z=74..77, so camera_z=0 looked ~48k world units away and the mesh
        # severely under-subdivided.  Keep the default camera space aligned with
        # the emitted OBJ/world coordinates.
        b = self.t.zmap.bounds()
        if b is None or self.args.origin == 'absolute':
            return 0.0, 0.0
        min_x, min_z, max_x, max_z = b
        if self.args.origin == 'min-corner':
            return min_x * PATCH_SIZE_WORLD, min_z * PATCH_SIZE_WORLD
        if self.args.origin == 'open76-centered':
            return (min_x + max_x) * 0.5 * PATCH_SIZE_WORLD, (min_z + max_z) * 0.5 * PATCH_SIZE_WORLD
        return 0.0, 0.0

    def lookup_sample_for_corner(self, wx: int, wz: int, root_x0: int, root_z0: int, root_x1: int, root_z1: int) -> Tuple[int, int]:
        if self.args.boundary_sampling == 'runtime-strict':
            return wx, wz
        sx, sz = wx, wz
        cell_x, cell_z = root_x0 // PAGE_W, root_z0 // PAGE_H
        if wx == root_x1 and (cell_x + 1, cell_z) not in self.active_cells:
            sx = wx - 1
        if wz == root_z1 and (cell_x, cell_z + 1) not in self.active_cells:
            sz = wz - 1
        return sx, sz

    def get_vertex(self, sx: int, sz: int, wx: Optional[int] = None, wz: Optional[int] = None) -> int:
        if wx is None:
            wx = sx
        if wz is None:
            wz = sz
        ...
        self.vertices.append(Vertex(sx, sz, wx, wz, word & 0x0fff, word, word >> 13))
        idx = len(self.vertices) - 1
        self.vertex_by_sample[key] = idx
        return idx

    def get_edge(self, va: int, vb: int) -> int:
        # Keep edge records oriented.  The runtime flatten routines emit vertex_a
        # or vertex_b depending on forward/reverse traversal, so reusing an
        # oppositely-oriented edge corrupts polygon winding and vertex order.
        key = (va, vb)
    ...
    def canonicalize_face_boundary(self, face: List[int]) -> List[int]:
        # Diagnostic runtime flatten order can be orientation-mixed because the
        # exact runtime edge sharing/split scheduling is not fully reproduced yet.
        # For OBJ/Blender output, reorder the collected boundary vertices around
        # the leaf polygon center in terrain wx/wz space.  This preserves the
        # adaptive boundary vertices but stabilizes winding and normals.
        uniq: List[int] = []
        seen = set()
        for vi in face:
            if vi not in seen:
        ...
        if len(uniq) < 3:
            return uniq
        cx = sum(self.vertices[vi].wx for vi in uniq) / len(uniq)
        cz = sum(self.vertices[vi].wz for vi in uniq) / len(uniq)
        ordered = sorted(uniq, key=lambda vi: math.atan2(self.vertices[vi].wz - cz, self.vertices[vi].wx - cx))
        # Ensure a consistent positive signed area in wx/wz.
        area = 0.0
        for a, b in zip(ordered, ordered[1:] + ordered[:1]):
            va, vb = self.vertices[a], self.vertices[b]
            area += va.wx * vb.wz - vb.wx * va.wz
        if area < 0.0:
            ordered.reverse()
        return ordered
    ...
    def should_split(self, x0: int, z0: int, x1: int, z1: int, depth: int) -> Tuple[bool, int]:
        ...
        cx = ((x0 + x1) * 0.5) * SAMPLE_SIZE_WORLD - self.split_origin_x
        cz = ((z0 + z1) * 0.5) * SAMPLE_SIZE_WORLD - self.split_origin_z
    ...
    def corner_vertex(self, wx: int, wz: int, root_x0: int, root_z0: int, root_x1: int, root_z1: int) -> int:
        sx, sz = self.lookup_sample_for_corner(wx, wz, root_x0, root_z0, root_x1, root_z1)
        return self.get_vertex(sx, sz, wx, wz)

    def prelink_split_edges(self, x0: int, z0: int, x1: int, z1: int, root_x0: int, root_z0: int, root_x1: int, root_z1: int) -> None:
        xm, zm = (x0 + x1) // 2, (z0 + z1) // 2
        v00 = self.corner_vertex(x0, z0, root_x0, root_z0, root_x1, root_z1)
        v10 = self.corner_vertex(x1, z0, root_x0, root_z0, root_x1, root_z1)
        v11 = self.corner_vertex(x1, z1, root_x0, root_z0, root_x1, root_z1)
        v01 = self.corner_vertex(x0, z1, root_x0, root_z0, root_x1, root_z1)
        vt = self.corner_vertex(xm, z0, root_x0, root_z0, root_x1, root_z1)
        vr = self.corner_vertex(x1, zm, root_x0, root_z0, root_x1, root_z1)
        vb = self.corner_vertex(xm, z1, root_x0, root_z0, root_x1, root_z1)
        vl = self.corner_vertex(x0, zm, root_x0, root_z0, root_x1, root_z1)
        ...
        self.corner_vertex(xm, zm, root_x0, root_z0, root_x1, root_z1)

    def emit_leaf(self, x0: int, z0: int, x1: int, z1: int, depth: int, material: int, root_x0: int, root_z0: int, root_x1: int, root_z1: int) -> None:
        v00 = self.corner_vertex(x0, z0, root_x0, root_z0, root_x1, root_z1)
        v10 = self.corner_vertex(x1, z0, root_x0, root_z0, root_x1, root_z1)
        v11 = self.corner_vertex(x1, z1, root_x0, root_z0, root_x1, root_z1)
        v01 = self.corner_vertex(x0, z1, root_x0, root_z0, root_x1, root_z1)
        ...
        seen_in_face = set()
        for vi in out:
            if cleaned and cleaned[-1] == vi:
                continue
            if vi in seen_in_face:
                continue
            cleaned.append(vi)
            seen_in_face.add(vi)
        if len(cleaned) > 1 and cleaned[0] == cleaned[-1]:
            cleaned.pop()
        if len(cleaned) >= 3:
            if self.args.face_order == 'canonical-boundary':
                cleaned = self.canonicalize_face_boundary(cleaned)
    ...
    def subdivide(self, x0: int, z0: int, x1: int, z1: int, depth: int, root_x0: int, root_z0: int, root_x1: int, root_z1: int) -> None:
        self.stats['quad_tests'] += 1
        split, material = self.should_split(x0, z0, x1, z1, depth)
        if not split:
            self.emit_leaf(x0, z0, x1, z1, depth, material, root_x0, root_z0, root_x1, root_z1)
            return
        self.stats['quad_splits'] += 1
        self.prelink_split_edges(x0, z0, x1, z1, root_x0, root_z0, root_x1, root_z1)
        xm, zm = (x0 + x1) // 2, (z0 + z1) // 2
        self.subdivide(x0, z0, xm, zm, depth + 1, root_x0, root_z0, root_x1, root_z1)
        self.subdivide(xm, z0, x1, zm, depth + 1, root_x0, root_z0, root_x1, root_z1)
        self.subdivide(xm, zm, x1, z1, depth + 1, root_x0, root_z0, root_x1, root_z1)
        self.subdivide(x0, zm, xm, z1, depth + 1, root_x0, root_z0, root_x1, root_z1)

    def build(self) -> None:
        for cell_x, cell_z, _ in self.t.zmap.used_cells():
            x0, z0 = cell_x * PAGE_W, cell_z * PAGE_H
            self.subdivide(x0, z0, x0 + PAGE_W, z0 + PAGE_H, 0, x0, z0, x0 + PAGE_W, z0 + PAGE_H)
    ...
    def stats_dict(self) -> dict:
        ...
        return {
            ...
            'face_order': self.args.face_order,
            'split_origin_world': [self.split_origin_x, self.split_origin_z],
            'camera_world_origin_relative': [self.args.camera_x, self.args.camera_z],
        }
...
def vertex_coord(v: Vertex, origin_x: float, origin_z: float, vertical_scale: float, mode: str) -> Tuple[float, float, float]:
    wx = v.wx * SAMPLE_SIZE_WORLD - origin_x
    wz = v.wz * SAMPLE_SIZE_WORLD - origin_z
...
def write_obj(path: Path, mission: Path, ter: Path, zmap: ZMap, b: AdaptiveBuilder, args: argparse.Namespace) -> None:
    ...
    with path.open('w', encoding='utf-8') as f:
        f.write("# Interstate '76 runtime-shaped adaptive terrain OBJ v4\n")
...
def out_dir_for(args: argparse.Namespace) -> Path:
    if args.out_dir is not None:
        return args.out_dir
    return args.default_root / time.strftime('i76_runtime_adaptive_terrain_v4_%Y%m%d_%H%M%S')
...
def export(args: argparse.Namespace) -> dict:
    ...
    manifest = {
        'format': 'i76_runtime_adaptive_terrain_export_manifest', 'version': 4,
        ...
        'runtime_approximate': [
            ...
            'boundary sampling is export-stabilized by default; use --boundary-sampling runtime-strict to reproduce literal lookup at x1/z1',
            'adaptive split predicate uses origin-relative camera/world coordinates by default; exact 004911e0 predicate remains unverified',
        ],
    ...
    for mission in sorted(missions):
        try:
            ...
            builder.build()
            obj_path = obj_dir / f'{mission.stem.lower()}.runtime_adaptive_v4.obj'
    ...
    manifest_path = out_dir / 'i76_runtime_adaptive_terrain_export_manifest_v4.json'
...
def parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Export Interstate '76 runtime-shaped adaptive terrain OBJ diagnostics v4 from miss8/miss16 missions.")
    ...
    ap.add_argument('--boundary-sampling', choices=['clamp-outer-active-edge', 'runtime-strict'], default='clamp-outer-active-edge',
                    help='runtime-strict samples x1/z1 literally; clamp mode preserves 128-sample patch span but samples last local TER sample when no active neighbor exists')
    ap.add_argument('--face-order', choices=['canonical-boundary', 'runtime-flatten'], default='canonical-boundary',
                    help='canonical-boundary stabilizes OBJ winding; runtime-flatten preserves the current diagnostic 00491a30 flatten order')
