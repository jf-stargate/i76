#!/usr/bin/env python3
"""
Interstate '76 Open76-aligned terrain OBJ exporter, v5.

Fixes vs earlier OBJ preview exporters:
  - Emits each terrain patch as 129x129 vertices, not 128x128.
    This removes the 5-meter gap between adjacent 640-meter terrain patches.
  - Fills the +X/+Z edge samples from neighbor patches, matching Open76's
    MsnMissionParser logic. If no neighbor exists, edge handling is configurable.
  - Recenters the output around the mission's active patch bounds by default,
    matching Open76's worldGameObject offset behavior.
  - Supports OBJ coordinate output as either:
      blender: v X Z HEIGHT   (native Blender Z-up if imported without axis conversion)
      y_up:    v X HEIGHT Z   (classic OBJ/Y-up convention)
  - Uses Open76's default vertical scale: low12 * (409.5 / 4096.0).

Input:
  Directory containing .MSN/.LVL plus matching .TER sidecars.

Example:
  python3 i76_level_blender_export_open76_v5.py "game/interstate 76/miss16" \
    --out-dir scratch_data/i76_blender_pkg_open76_v5 \
    --mission-filter m05 \
...
DEFAULT_SAMPLE_SCALE = PATCH_SIZE_WORLD / PAGE_W  # 5.0
...
def stitched_sample(
    ...
    # Neighbor stitching matching Open76's intent:
    # h[*,128]     = right patch first column
    # h[128,*]     = bottom patch first row
    # h[128,128]   = bottom-right patch [0,0]
    if local_x == PAGE_W and local_z < PAGE_H:
...
def convert_vertex(wx: float, wz: float, h: float, obj_coordinates: str) -> Tuple[float, float, float]:
    if obj_coordinates == "blender":
        # Blender-native: Z is up.
        return wx, wz, h
    if obj_coordinates == "y-up":
        # Traditional OBJ convention: Y is up.
        return wx, h, wz
...
def write_mission_obj(
    ...
    *,
    ...
    used = zmap.used_cells()

    vertex_count = 0
    face_count = 0

    with obj_path.open("w", encoding="utf-8") as f:
        f.write("# Interstate '76 Open76-aligned stitched terrain OBJ v5\n")
        ...
        f.write("# patch vertices are 129x129 to span exactly 640 world units\n")

        # Emit one OBJ group per patch, but keep all vertices in one file.
        for cell_x, cell_z, page_idx in used:
            f.write(f"g patch_x{cell_x}_z{cell_z}_page{page_idx}\n")
            base_vertex = vertex_count + 1

            for local_z in range(PAGE_H + 1):
                for local_x in range(PAGE_W + 1):
                    word = stitched_sample(pages, zmap, cell_x, cell_z, local_x, local_z, edge_mode)
                    height = (word & 0x0FFF) * vertical_scale
                    wx = cell_x * PATCH_SIZE_WORLD + local_x * DEFAULT_SAMPLE_SCALE - offset_x
                    wz = cell_z * PATCH_SIZE_WORLD + local_z * DEFAULT_SAMPLE_SCALE - offset_z
                    vx, vy, vz = convert_vertex(wx, wz, height, obj_coordinates)
                    f.write(f"v {vx:.6f} {vy:.6f} {vz:.6f}\n")
                    vertex_count += 1

            for local_z in range(PAGE_H):
                for local_x in range(PAGE_W):
                    v0 = base_vertex + local_z * (PAGE_W + 1) + local_x
                    f.write(f"f {v0} {v0 + 1} {v0 + PAGE_W + 2} {v0 + PAGE_W + 1}\n")
                    face_count += 1
    ...
    return {
        ...
        "faces": face_count,
        "origin_offset": [offset_x, offset_z],
...
def export(input_path: Path, out_dir: Path, mission_filter: str, origin_mode: str, obj_coordinates: str, edge_mode: str, vertical_scale: float, fail_empty: bool) -> dict:
    ...
    manifest = {
        "format": "i76_open76_terrain_obj_manifest",
        "version": 5,
    ...
    manifest_path = out_dir / "i76_open76_terrain_obj_manifest_v5.json"
...
def main() -> int:
    ...
    ap.add_argument("--mission-filter", default="", help="Substring filter, e.g. m05, s05, t05")
    ...
    ap.add_argument("--edge-mode", choices=["clamp", "zero"], default="clamp",
                    help="Use clamp for clean preview edges; zero more closely matches Open76 defaultHeights at missing neighbors.")
