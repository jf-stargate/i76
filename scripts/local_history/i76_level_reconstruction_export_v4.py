#!/usr/bin/env python3
"""
Interstate '76 level reconstruction exporter, v4.
...
Fixes in v4:
  - RSEG OBJ vertical coordinate no longer uses serialized float +0x10 by default.
    That field can contain endpoint/control values, causing huge vertical spikes.
    RSEG probes now default to sampling the runtime terrain height at each road point.

Fixes in v2/v3:
  - ODEF and RSEG overlay coordinates are now transformed into the same mission-origin
    frame as terrain OBJ output. v1 emitted those probes in raw mission/world coordinates.
  - Adaptive overlay is explicitly documented as diagnostic; it does not yet share split
    edges across neighboring quads/pages the way the runtime edge-record graph does.
...
Example:
  python3 i76_level_reconstruction_export_v3.py \
...
def origin_world_for_zmap(zmap: ZMap, origin_mode: str) -> Tuple[float, float]:
    sx, sz = origin_samples_for_zmap(zmap, origin_mode)
    return sx * SAMPLE_WORLD_SCALE, sz * SAMPLE_WORLD_SCALE
...
def write_odef_axes_obj(path: Path, records: List[dict], obj_coordinates: str,
                        include_invalid: bool, axis_len: float = 25.0,
                        origin_world_x: float = 0.0, origin_world_z: float = 0.0) -> dict:
    path.parent.mkdir(parents=True, exist_ok=True)
    vertices: List[Tuple[float, float, float]] = []
...
def write_rseg_obj(path: Path, points: List[dict], mode: str, obj_coordinates: str,
                   origin_world_x: float = 0.0, origin_world_z: float = 0.0,
                   rt: Optional[RuntimeTerrain] = None,
                   height_mode: str = "terrain", height_offset: float = 1.0) -> dict:
    path.parent.mkdir(parents=True, exist_ok=True)
    vertices: List[Tuple[float, float, float]] = []
    lines: List[Tuple[int, int]] = []
    ...
    def road_height(x: float, z: float, serialized_h: float = 0.0) -> float:
        ...
        if height_mode == "terrain":
            if rt is None:
                return height_offset
            # RSEG x/z are in mission/world units. Terrain lookup uses sample units.
...
def export(args: argparse.Namespace) -> dict:
    ...
    out_dir = timestamped_out_dir(args.out_dir, "i76_level_reconstruction_export_v3")
    ...
    manifest = {
        ...
        "runtime_approximate": [
            ...
            "RSEG serialized float +0x10 meaning; v4 does not use it as vertical height by default",
            ...
            if args.write_rseg and rseg_points:
                ...
                for mode in ("centerline", "left_edge", "right_edge", "crossbars"):
                    obj_info = write_rseg_obj(
                        ...
                        origin_world_z,
                        rt,
                        args.rseg_height_mode,
                        args.rseg_height_offset,
                    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--rseg-height-mode", choices=["terrain", "zero", "serialized"], default="terrain",
                    help="Vertical coordinate for RSEG probes. 'terrain' samples .TER height; 'serialized' uses RSEG float +0x10 and is diagnostic only.")
    ap.add_argument("--rseg-height-offset", type=float, default=1.0,
                    help="Vertical lift applied to RSEG probes in terrain/zero height modes so lines sit above terrain.")
