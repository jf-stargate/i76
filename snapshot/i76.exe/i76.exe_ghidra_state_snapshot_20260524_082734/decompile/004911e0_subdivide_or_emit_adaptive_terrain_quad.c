...
/* [i76 level/static/material baseline v48]
   name: adaptive_terrain_should_subdivide_patch
   confidence: high
   module: terrain_lod
   Adaptive terrain subdivision predicate. Confirmed hard split while half_scale >= 0x20 and
   special-surface forced split when word >= 0x4000 && (word & 0xe000) != 0xa000.
