...
/* [i76 level/static/material baseline v48]
   name: load_vehicle_config_for_world_object
   confidence: high
   module: vehicle_vcf
   Loads VCF-backed world object/vehicle config. ODEF class 0x01/0x08/0x09 use label.vcf except
   spawn/regen special markers.
...

{
  ...
  *(byte **)(param_2 + 4) = local_aa8;
  ...
  bVar3 = parse_bwd2_resource_with_descriptor_table
                    (local_cbc,&g_vcfc_descriptor_table_candidate,4,param_2);
  ...
  if (g_selected_mission_family == 2) {
    ...
    if (CONCAT31(extraout_var_00,bVar3) != 0) {
      parse_bwd2_resource_with_descriptor_table
                (local_cbc,&g_vehicle_geometry_descriptor_table_candidate,5,param_2);
      return 1;
    }
  }
  return 1;
