...
/* [i76 level/static/material baseline v48]
   name: handle_ldef_obj_static_scene_object
   confidence: high
   module: level_object_dispatch
   LDEF/OBJ static scene-object handler. Formats/resolves label.sdf for normal records; distinct
   from ODEF class-specific dispatch.
...

{
  ...
  pfVar15 = g_runtime_terrain_vertex_xyz_buffer;
  ...
  local_1c0 = g_runtime_terrain_vertex_xyz_buffer;
  local_1c4 = g_runtime_terrain_vertex_xyz_buffer + 3;
  local_1d0 = *g_runtime_terrain_vertex_xyz_buffer;
  local_1cc = g_runtime_terrain_vertex_xyz_buffer[1];
  local_1c8 = g_runtime_terrain_vertex_xyz_buffer[2];
  ...
  *(double *)(local_154[0] + 0x14) = (double)local_1c8;
  ...
  while( true ) {
    ...
    local_1c4 = (float *)0x0;
    if (local_14c != 0) {
      iVar9 = bind_odef_vehicle_geometry_context(local_148);
    }
