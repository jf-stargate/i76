/*
 * Program: i76.exe
 * Function: split_terrain_edge_midpoint
 * Entry: 004918f0
 * Signature: undefined __cdecl split_terrain_edge_midpoint(undefined2 * param_1)
 */
...
/* cgpt label refinement v19: was terrain_map_runtime_terrain_map_context_helper_004918f0. Remove
   ...
   I76 rename v43: split_terrain_edge_midpoint
   Splits/reuses terrain edge midpoint record. */

void __cdecl split_terrain_edge_midpoint(undefined2 *param_1)

{
  ...
  if (*(int *)(param_1 + 2) == 0) {
    ...
    *(float *)(g_runtime_terrain_vertex_xyz_buffer + 8 + iVar10) = fVar3;
    uVar7 = sample_runtime_terrain_height(iVar9 >> 1,iVar8 >> 1);
    *(float *)(g_runtime_terrain_vertex_xyz_buffer + 4 + iVar10) =
         (float)CONCAT22(extraout_var,uVar7);
    *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer + iVar10) =
         fVar2 * _DAT_004be80c;
    *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer + 4 + iVar10) =
         fVar3 * _DAT_004be80c;
