...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 4 | `tag` | `char[4]` | FourCC chunk tag. |
| 1 | 4 | 4 | `arg_or_extension_ptr` | `dword` | Descriptor-specific argument pointer, extension pointer, or count storage depending on flags. |
| 2 | 8 | 4 | `callback` | `pointer` | Chunk handler callback. |
| 3 | 12 | 4 | `flags` | `dword` | Parser flags controlling mandatory/repeating/count/argument behavior. |
...
## `i76_runtime_ter_packed_word_v1`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 4 | `documentation_only` | `dword` | Runtime .TER page samples are uint16, not dword. low 12 bits height; bit12 surface flag; bits13..15 surface class. |

## `i76_runtime_terrain_edge_record`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 2 | `vertex_a` | `short` | Endpoint vertex index A. |
| 1 | 2 | 2 | `vertex_b` | `short` | Endpoint vertex index B. |
| 2 | 4 | 4 | `child_edge_a` | `pointer` | First child after midpoint split; null if leaf. |
| 3 | 8 | 4 | `child_edge_b` | `pointer` | Second child after midpoint split; null if leaf. |

## `i76_runtime_terrain_polygon_work_header`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 4 | `vertex_count` | `dword` | Number of polygon vertices. Zero means fixed-size record path. |
| 1 | 4 | 4 | `material_or_type` | `dword` | Polygon material/type. Software path uses 0xb4; alt path uses DAT_0059c558. |
| 2 | 8 | 4 | `flags` | `dword` | Polygon flags/material flags. |
| 3 | 12 | 4 | `record_type` | `dword` | Record type/class used by scan binning; initially cleared before clip/prep. |
| 4 | 16 | 4 | `max_or_sort_depth` | `float` | Sort depth/range used by scan bucket insertion. |
| 5 | 20 | 4 | `next` | `pointer` | Next polygon work record in bucket list. |
| 6 | 24 | 4 | `prev` | `pointer` | Previous polygon work record in bucket list. |

## `i76_runtime_terrain_polygon_work_vertex`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 4 | `x` | `float` | World/render X copied from generated terrain vertex. |
| 1 | 4 | 4 | `y` | `float` | World/render Y or height copied from generated terrain vertex. |
| 2 | 8 | 4 | `z` | `float` | World/render Z/depth copied from generated terrain vertex. |
| 3 | 12 | 4 | `screen_or_projected_x` | `float` | Alternate path projected/scaled X; software path may leave/clip-fill. |
| 4 | 16 | 4 | `screen_or_projected_y` | `float` | Alternate path projected/scaled Y; software path may leave/clip-fill. |
| 5 | 20 | 4 | `shade_or_depth` | `float` | Shade/intensity or per-vertex depth value. |

## `i76_runtime_terrain_primitive_node`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 2 | `screen_or_bucket_start` | `short` | ftol-derived range/screen start field. |
| 1 | 2 | 2 | `screen_or_bucket_end` | `short` | ftol-derived range/screen end field. |
| 2 | 4 | 4 | `range_delta` | `dword` | ftol-derived end-start/range delta. |
| 3 | 8 | 4 | `next` | `pointer` | Next primitive in generated adaptive-terrain primitive linked list. |
| 4 | 12 | 4 | `edge_ref_0` | `pointer` | First terrain edge reference. |
| 5 | 16 | 4 | `edge_ref_1` | `pointer` | Second terrain edge reference. |
| 6 | 20 | 4 | `edge_ref_2` | `pointer` | Third terrain edge reference. |
| 7 | 24 | 4 | `edge_ref_3` | `pointer` | Fourth terrain edge reference. |

## `i76_runtime_terrain_root_edge_seed_record`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 12 | `edge_record` | `short[6]` | Same physical 0x0c edge record shape used for root edges. |

## `i76_runtime_terrain_scan_bucket_header`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 4 | `head` | `pointer` | Head polygon/span entry pointer. |
| 1 | 4 | 4 | `tail` | `pointer` | Tail polygon/span entry pointer. |
| 2 | 8 | 4 | `count` | `dword` | Entry count in bucket. |

## `i76_runtime_terrain_scan_bucket_span_entry`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 4 | `left_or_min` | `float` | Span left/min bound. |
| 1 | 4 | 4 | `right_or_max` | `float` | Span right/max bound. |
| 2 | 8 | 4 | `depth_bucket` | `dword` | Depth bucket/range index. |
| 3 | 12 | 4 | `next` | `pointer` | Next span entry. |

## `i76_runtime_terrain_vertex_xyz`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 4 | `x` | `float` | Generated/transformed terrain X. |
| 1 | 4 | 4 | `y_or_height` | `float` | Generated/transformed terrain height/Y. |
| 2 | 8 | 4 | `z_or_depth` | `float` | Generated/transformed terrain Z/depth. |

## `i76_runtime_zmap_payload_v1`
...
| Ordinal | Offset | Length | Name | Type | Comment |
|---:|---:|---:|---|---|---|
| 0 | 0 | 1 | `active_zone_count` | `char` | Number of active .TER zone pages. |
| 1 | 1 | 6400 | `zone_id_map_80x80` | `char[6400]` | Zone id per 640m world grid cell; values must be < active_zone_count. |
