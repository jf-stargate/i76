...

{
  ...
  if (iVar9 == g_zix_search_path_count) {
    ...
    pcVar8 = &g_zix_search_path_records + g_zix_search_path_count * 0x10c;
  ...
  pvVar5 = HeapReAlloc(g_process_heap,0,g_registered_resource_records,
                       (g_registered_resource_record_count + 1) * 0x30);
  if (pvVar5 == (LPVOID)0x0) {
    fatal_out_of_memory_message((LPCSTR)0x0);
  }
  pcVar8 = (char *)(g_registered_resource_record_count * 0x30 + (int)pvVar5);
  g_registered_resource_record_count = g_registered_resource_record_count + 1;
  pcVar10 = pcVar8;
  g_registered_resource_records = pvVar5;
  ...
  *(uint *)(pcVar8 + (iVar3 + 4) * 4) =
       *(uint *)(pcVar8 + (iVar3 + 4) * 4) |
       1 << ((((byte)iVar9 ^ bVar2) - bVar2 & 0x1f ^ bVar2) - bVar2 & 0x1f);
  qsort(g_registered_resource_records,g_registered_resource_record_count,0x30,
        (_PtFuncCompare *)&compare_resource_record_16byte_key);
  return 1;
}
