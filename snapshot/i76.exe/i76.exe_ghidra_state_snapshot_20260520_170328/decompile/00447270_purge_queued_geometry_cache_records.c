...

{
  ...
  while (g_geo_constructor_allocation_list_head != (uint *)0x0) {
    uVar5 = (g_geo_constructor_allocation_list_head[3] * 0x6cd + 0xaab) % 0x7ed;
    puVar3 = &g_geometry_cache_by_handle + uVar5;
    for (puVar1 = (uint *)(&g_geometry_cache_by_handle)[uVar5];
        puVar1 != g_geo_constructor_allocation_list_head; puVar1 = (uint *)puVar1[4]) {
      puVar3 = puVar1 + 4;
    }
    *puVar3 = puVar1[4];
    uVar5 = *puVar1;
    ...
    for (puVar1 = (uint *)(&g_geometry_cache_by_8char_name)[uVar5];
        puVar1 != g_geo_constructor_allocation_list_head; puVar1 = (uint *)puVar1[5]) {
      puVar3 = puVar1 + 5;
    }
    *puVar3 = puVar1[5];
    ...
    SVar4 = HeapSize(g_geometry_heap_handle,0,lpMem);
    g_geo_runtime_heap_bytes_tracked = g_geo_runtime_heap_bytes_tracked + SVar4;
    HeapFree(g_geometry_heap_handle,0,lpMem);
    HeapFree(g_geometry_cache_entry_heap,0,puVar1);
  }
  piVar6 = &g_geometry_cache_by_handle;
  iVar7 = 0;
  g_geo_constructor_allocation_list_head = (uint *)iVar7;
  do {
    for (iVar2 = *piVar6; iVar2 != 0; iVar2 = *(int *)(iVar2 + 0x10)) {
      SVar4 = HeapSize(g_geometry_heap_handle,0,*(LPCVOID *)(iVar2 + 0xc));
      if (0 < (int)SVar4) {
      ...
      iVar7 = (int)g_geo_constructor_allocation_list_head;
    }
    piVar6 = piVar6 + 1;
  } while ((int)piVar6 < 0x52fb5c);
  for (; iVar7 != 0; iVar7 = *(int *)(iVar7 + 0x18)) {
    SVar4 = HeapSize(g_geometry_heap_handle,0,*(LPCVOID *)(iVar7 + 0xc));
    if (0 < (int)SVar4) {
      uVar5 = SVar4 + 0xfff >> 0xc;
      do {
        uVar5 = uVar5 - 1;
