/*
 * Program: i76.exe
 * Function: build_adaptive_terrain_root_primitives
 * Entry: 00490a00
 * Signature: undefined __cdecl build_adaptive_terrain_root_primitives(int param_1)
 */
...
/* cgpt label refinement v19: was terrain_map_runtime_terrain_map_context_helper_00490a00. Remove
   ...
   I76 rename v43: build_adaptive_terrain_root_primitives
   Seeds adaptive terrain root vertices/edges/primitives. */

void __cdecl build_adaptive_terrain_root_primitives(int param_1)

{
  ...
  _g_runtime_terrain_lod_error_threshold_z =
  ...
  _g_runtime_terrain_lod_error_threshold_x =
  ...
  iVar12 = terrain_runtime_lookup_context_helper(param_1);
  ...
  *(float *)(g_runtime_terrain_vertex_xyz_buffer + 8 + iVar12) = fVar2;
  uVar10 = sample_runtime_terrain_height(iVar13,iVar16);
  *(float *)(g_runtime_terrain_vertex_xyz_buffer + 4 + iVar12) =
       (float)CONCAT22(extraout_var_01,uVar10);
  *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer + iVar12) =
       fVar1 * _DAT_004be80c;
  *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer + 4 + iVar12) =
       fVar2 * _DAT_004be80c;
  ...
  *(float *)(g_runtime_terrain_vertex_xyz_buffer + 8 + iVar14) = fVar2;
  uVar10 = sample_runtime_terrain_height(iVar12,iVar13);
  *(float *)(g_runtime_terrain_vertex_xyz_buffer + 4 + iVar14) =
       (float)CONCAT22(extraout_var_02,uVar10);
  *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer + iVar14) =
       fVar1 * _DAT_004be80c;
  *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer + 4 + iVar14) =
       fVar2 * _DAT_004be80c;
  ...
  *(float *)(g_runtime_terrain_vertex_xyz_buffer + 8 + iVar15) = fVar2;
  uVar10 = sample_runtime_terrain_height(iVar14,iVar13);
  *(float *)(g_runtime_terrain_vertex_xyz_buffer + 4 + iVar15) =
       (float)CONCAT22(extraout_var_03,uVar10);
  *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer + iVar15) =
       fVar1 * _DAT_004be80c;
  *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer + 4 + iVar15) =
       fVar2 * _DAT_004be80c;
  ...
  *(float *)(g_runtime_terrain_vertex_xyz_buffer + iVar15) = fVar1;
  fVar2 = (float)iVar17;
  *(float *)(g_runtime_terrain_vertex_xyz_buffer + 8 + iVar15) = fVar2;
  uVar10 = sample_runtime_terrain_height(iVar13,iVar17);
  *(float *)(g_runtime_terrain_vertex_xyz_buffer + 4 + iVar15) =
  ...
  *g_runtime_terrain_arena_cursor = local_10;
  ...
  psVar7[5] = 0;
  g_runtime_terrain_edge_record_count = g_runtime_terrain_edge_record_count + 1;
  subdivide_or_emit_adaptive_terrain_quad(psVar7,psVar6,psVar5,psVar3);
  if (g_runtime_terrain_primitive_list_head != 0) {
    terrain_runtime_finalize_or_draw_helper();
    if ((g_terrain_cached_high_color_resource_mode != 0) &&
      ...
      do {
        local_24 = SUB84((double)(((float)_DAT_004faf18 -
                                  *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer
                                            + iVar16) * _DAT_004be810) - (float)_DAT_004be818),0);
        local_2c = SUB84((double)(((float)_DAT_004faf18 -
                                  *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer
                                             + 4 + iVar16) * _DAT_004be810) - (float)_DAT_004be818),
                         0);
        fVar18 = prepare_adaptive_terrain_render_context
                           (local_24,local_2c,
                            *(float *)(g_runtime_terrain_vertex_xyz_buffer + 8 + iVar16));
        iVar13 = iVar13 + 1;
        *(float *)(g_runtime_terrain_vertex_aux_projected_lighting_buffer + 8 + iVar16) =
             (float)fVar18;
        iVar16 = iVar16 + 0xc;
      } while (iVar13 < g_runtime_terrain_vertex_count);
    }
    if (g_player_options_terrain_alt_polygon_path == '\0') {
      flatten_terrain_primitive_edges_and_compute_planes
                (&LAB_0048e190,terrain_runtime_record_helper_004922f0);
      return;
    }
    flatten_terrain_primitive_edges_and_compute_planes
              (&LAB_004923f0,terrain_runtime_record_helper_004924b0);
  }
  return;
}
