...
/* cgpt readability rename v13 set A: High-level frame render pipeline with timing labels and
   ...
   [cgpt_i76exe_render_mode_v13] proposed=render_game_frame_d3d_or_hardware_pipeline_debug_timed ::
   High-level frame render path used for D3D/hardware-style pipeline; calls adaptive terrain builder
   and hardware-specific render passes. */
...

{
  ...
  local_8 = 0;
  build_adaptive_terrain_root_primitives((int)param_1);
  ...
  restore_or_acquire_directdraw_surface_if_needed();
  if ((g_cmdline_hardware_raster_requested == 0) && (g_runtime_high_color_resource_mode == 1)) {
