/*
 * Program: i76.exe
 * Function: update_runtime_input_state_and_pending_action
 * Entry: 0044d9e0
 * Signature: undefined __stdcall update_runtime_input_state_and_pending_action(void)
 */
...
void update_runtime_input_state_and_pending_action(void)

{
  ...
  int iStack_4;
                    ...
                    /* I76 semantic baseline: update_runtime_input_state_and_pending_action
                       Per-frame input binding update: axes, chord flags, device polling, pending
                       action code. */
  iVar10 = 0;
    ...
    if (0 < g_input_axis_binding_count) {
      ...
      do {
        if ((DAT_004f2848 != 0) || (*piVar12 != 0)) {
          iVar8 = update_one_axis_binding_from_latched_or_digital_input(piVar12 + -0x1f);
