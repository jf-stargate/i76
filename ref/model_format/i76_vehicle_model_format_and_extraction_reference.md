# Interstate ’76 Vehicle Model Format and Extraction Reference

This page documents the current reverse-engineered understanding of Interstate ’76 vehicle model extraction, including the resource chain from `VCF` through `VDF`/`VTF`/`GEO`, texture decoding, material binding, and the current binary-faithful OBJ export conventions.

Status: reverse-engineered from retail `i76.exe`, validated against the retail archive corpus, Open76 behavior where useful, and repeated visual checks in Blender.
...
- The known layouts of `VCF`, `VDF`, `VGEO`, `GEO`, `VTF`, `TMT`, `M16`, `VQM`, and `CBK`.
...
- Mission/world placement formats.
- Full `SDF`/scene-object model reconstruction.
- Complete weapon/equipment behavior.
- Audio, cockpit UI, FMV, or gameplay simulation.
- A complete specification of all chunk types not needed for vehicle visual extraction.

## High-Level Asset Chain

Vehicle extraction is not a single-file decode. A visible vehicle is assembled from several linked resources:

```text
+---------------------+---------------------------------+------------------------------------------------------------------------+
| Layer               | Format(s)                       | Role                                                                   |
+---------------------+---------------------------------+------------------------------------------------------------------------+
| Archive             | ZFSF / ZIX                      | Top-level resource storage and lookup/indexing                         |
| Embedded containers | PIX / PAK                       | Container records embedded in archive payloads                         |
| Vehicle config      | VCF / VCFC                      | Root vehicle resource; references VDF, VTF, WDF, components            |
| Vehicle definition  | VDF / VDFC / VGEO / WLOC / HLOC | Runtime vehicle definition, visible part hierarchy, wheels, hardpoints |
| Geometry            | GEO                             | Mesh vertices, normal table, faces, texture slots, UVs                 |
| Texture map set     | VTF / VTFC                      | Vehicle material slot table and body/six-face map references           |
| Texture animation   | TMT                             | Texture frame list, dimensions, animation flags                        |
| Texture image       | MAP / M16 / VQM / CBK           | Indexed texture payloads; VQM uses 4x4 codebook blocks                 |
| Palette             | ACT / runtime palette           | 256-color RGB palette for indexed texture preview/export               |
| Weapon/equipment    | WDF / GDF                       | Mounted weapons and equipment referenced by VCF/VDF                    |
+---------------------+---------------------------------+------------------------------------------------------------------------+
```

The practical chain for current vehicle extraction is:
...
VDF
  -> SOBJ/VGEO part names and transforms
  -> WLOC wheel transforms
  -> HLOC hardpoint/mount transforms
  -> collision and display metadata

VGEO/SOBJ part basename + ".geo"
  -> usually embedded in PIX/PAK and indexed by sibling PIX/PAK records
...
The current project names for the most important vehicle/model/texture functions are:

```text
+----------+---------------------------------------------------+----------------------------------------------------+
| Address  | Current name                                      | Role                                               |
+----------+---------------------------------------------------+----------------------------------------------------+
| 004ad640 | format_8char_resource_basename_with_suffix        | Build fixed-width basename + suffix resource names |
| 004ad6f0 | load_vehicle_config_for_world_object              | Top-level vehicle config loader                    |
| 004ad950 | parse_vcfc_vdf_vtf_wdf_resource_refs              | Parse VCF root references                          |
...
| 004b8470 | load_sobj_scene_object_geometry_id                | Scene-object SOBJ geometry loader                  |
...
The `VCF` file is the root vehicle configuration. It contains a `VCFC` chunk whose fixed fields reference the vehicle definition, texture map set, weapon definitions, and component selectors.
...
Binary evidence shows two distinct SOBJ concepts:

```text
Vehicle VDF descriptor table at 00500090:  SOBJ callback = 00000000
Scene/SDF descriptor table at 00500b18:    SOBJ callback = 004b8470
```

The function currently named `load_sobj_scene_object_geometry_id` at `004b8470` is a scene-object SOBJ geometry loader, not necessarily the vehicle VDF SOBJ path.
...
A durable vehicle extraction should emit both visual and machine-readable artifacts:
...
However, Open76 should not be adopted wholesale for Blender OBJ output. A broad Open76 parity rewrite with raw V, inverted normals, and Open76 triangle fan order produced incorrect texture orientation in the Blender OBJ path.

Use Open76 as corroborating evidence, not as the final export convention.
...
| SOBJ auxiliary handling              | Medium-high             | Binary distinguishes vehicle VDF SOBJ vs scene SOBJ callback        |
| Exact non-vehicle model rules        | Medium                  | Likely similar GEO rules, but vehicle path is best validated        |
+--------------------------------------+-------------------------+---------------------------------------------------------------------+
```
...
1. Run the v13 baseline across all vehicles and inspect in Blender grid view.
2. Record per-vehicle exceptions for alpha/light overlays rather than broadening alpha rules prematurely.
3. Extend the durable snapshot pages with representative TSV rows for known-good Piranha body/light faces.
4. Validate non-vehicle scene-object GEOs against the same runtime GEO rules.
...
6. Build a standalone extraction pipeline that does not depend on the older OBJ exporter helper except for validated reusable decoding code.
