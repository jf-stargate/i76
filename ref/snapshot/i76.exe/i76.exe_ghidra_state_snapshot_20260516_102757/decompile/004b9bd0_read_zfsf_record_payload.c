...

{
  ...
  if ((bVar6) || (param_4 == (uint *)0x0)) {
    puVar5 = HeapAlloc(g_process_heap,0,(SIZE_T)this);
    local_30 = puVar5;
    if (puVar5 == (uint *)0x0) {
  ...
  if (sVar3 == 0) {
    sprintf(&g_resource_error_message,s_Could_not_read_record__s_from_fi_00500f2c,param_2,
            param_1 + 1);
    if (puVar5 == (uint *)0x0) {
      return (uint *)0x0;
    }
    HeapFree(g_process_heap,0,puVar5);
    return (uint *)0x0;
  }
  if (bVar6) {
    dwBytes = *(int *)((int)pvVar2 + 0x20) >> 8;
    if (param_4 == (uint *)0x0) {
      param_4 = HeapAlloc(g_process_heap,0,dwBytes);
      if (param_4 == (uint *)0x0) {
        fatal_out_of_memory_message((LPCSTR)0x0);
      }
    }
