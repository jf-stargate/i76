...
| `00409850` | `register_vehicle_fsm_runtime_for_world_object` | plate | cgpt rename v10 fsm/ai runtime core cluster: World-object registration callback: inserts vehicle/actor objects into team buckets, initializes vehicle damage/FSM state, parses tactics tag, and caches terrain material. |
...
| `0040b0f0` | `fsm_query_who_shot` | plate | i76 first-pass rename<br>old_name: FUN_0040b0f0<br>suggested_name: fsm_query_who_shot<br>basis: FSM diagnostic string indicates whoShot query when nobody has shot yet. |
...
| `00438fd0` | `integrate_vehicle_physics_and_wheel_contacts` | plate | [cgpt i76.exe vehicle suspension/contact renames v32; confidence=high] Top-level vehicle physics integrator for velocity, transform, contact flags, wheel/tire child state, tire speed scalars, airborne/grounded state, and impact damage. |
...
| `0043d070` | `update_vehicle_recovery_or_autopilot_state` | plate | [cgpt i76.exe vehicle suspension/contact renames v32; confidence=medium] Vehicle recovery/autopilot-like runtime update helper in the physics step path. |
...
| `0044d780` | `input_device_binding_input_action_state_helper_0044d780` | plate | cgpt readability rename set B v14: Readability pass set B: likely input_device_binding / input_action_state_helper based on address neighborhood and prior focused closure context. |
...
| `0044d9e0` | `update_input_bindings_and_pending_action_code` | plate | [cgpt i76.exe runtime loop refinement v23; confidence=HIGH] Early per-frame input update. Polls device callbacks, evaluates condition/axis bindings, updates bound axis values, and writes g_pending_input_action_code. |
| `0044dcc0` | `input_device_binding_input_action_state_helper_0044dcc0` | plate | cgpt readability rename set B v14: Readability pass set B: likely input_device_binding / input_action_state_helper based on address neighborhood and prior focused closure context. |
| `0044de10` | `clear_pending_input_action_state` | plate | cgpt rename v2: Clears a pending input/action global used by the input dispatcher and player death path. |
...
| `0044dec0` | `dispatch_pending_input_action_bindings` | plate | [cgpt i76.exe runtime loop refinement v23; confidence=HIGH] Dispatches the currently pending input action through binding/action handler records. |
...
| `0044e010` | `is_runtime_input_or_update_paused` | plate | [cgpt i76.exe vehicle mainloop physics rename v30; confidence=medium] Pause/update gate checked before running the direct vehicle frame update branch. |
...
| `00461c80` | `dispatch_world_object_frame_begin_callbacks` | plate | [cgpt i76.exe runtime loop refinement v23; confidence=HIGH] Per-frame world-object callback pass before major simulation update. |
| `00461d50` | `dispatch_world_object_frame_end_callbacks` | plate | [cgpt i76.exe runtime loop refinement v23; confidence=HIGH] Per-frame world-object callback pass after major simulation update. |
...
| `004639a0` | `release_vehicle_runtime_context_and_components` | plate | [cgpt i76.exe vehicle class physics rename v29; confidence=high] Class-id 1 release callback. Releases child/component world objects for engine/brake/suspension/wheels/special parts and then releases vehicle runtime context. |
| `00463a80` | `apply_player_vehicle_damage_context_and_death` | plate | [cgpt i76.exe vehicle class physics rename v29; confidence=high] Class-id 1 damage callback. Applies vehicle damage, updates texture state, handles death/kill messages, and removes the loaded vehicle runtime when destroyed. |
...
| `0049c7c0` | `get_runtime_time_seconds` | plate | [cgpt i76.exe damage runtime deeptrace v20; confidence=high] Returns runtime time seconds global used by damage/effect timestamps. |
...
| `0049c7f0` | `elapsed_time_or_timer_update` | plate | i76 first-pass rename<br>old_name: FUN_0049c7f0<br>suggested_name: elapsed_time_or_timer_update<br>basis: Timing/update helper using GetTickCount. |
...
| `0049c8c0` | `get_multiplayer_or_runtime_seconds` | plate | [cgpt i76.exe main runtime/FSM rename v21; confidence=MEDIUM] Returns the float runtime/network timer at 005a7e74; heavily used in multiplayer timing gates.<br>old_name: FUN_0049c8c0 |
| `0049c8d0` | `mission_flow_map_context_helper_0049c8d0` | plate | cgpt label refinement v20: was mission_flow_map_camera_mission_flow_or_map_helper_0049c8d0. Remove duplicated mission-flow/map wording. |
| `0049c920` | `get_tick_count_wrapper_c` | plate | i76 first-pass rename<br>old_name: FUN_0049c920<br>suggested_name: get_tick_count_wrapper_c<br>basis: Small GetTickCount wrapper in utility/timing area. |
