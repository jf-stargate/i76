/*
 * Program: i76.exe
 * Function: activate_runtime_camera_mode_18_weapon_target_view
 * Entry: 00408b50
 * Signature: undefined __cdecl activate_runtime_camera_mode_18_weapon_target_view(int * param_1)
 */


/* cgpt rename v5 runtime camera cluster: Runtime camera mode table id 0x18; switches into
   weapon-target tracking view when the tracked vehicle has a primary weapon target. */

void __cdecl activate_runtime_camera_mode_18_weapon_target_view(int *param_1)

{
  ...
  if (uVar1 != 0) {
    ...
    g_runtime_display_mode_change_callback = update_runtime_camera_weapon_target_tracking_callback;
  }
  return;
}
